...
class ReflectionShortestPath(LessonScene):
    ...
    def _pt(self, wx, wy):
        return np.array([(wx - 4.0) * self.SCALE, wy * self.SCALE, 0.0])
    ...
    def part_question(self):
        fig = self._base_figure(p_x=4.0, show_b_prime=False, path_opacity=0.55)
        fig.next_to(self.header, DOWN, buff=0.4)
        fig.set_x(0)
        self.play(FadeIn(fig), run_time=0.9)
        self.linger(3.2)

        q1 = self._line("川に一度だけ触れて、", MathTex(r"A", font_size=28), "から", MathTex(r"B", font_size=28), "へ行きます。", font_size=26)
    ...
    def part_trial_near_b(self):
        ...
        fig = self._base_figure(p_x=8.0, show_b_prime=False)
        ...
        rows = [
            ...
            MathTex(r"AP=\sqrt{8^2+6^2}=\sqrt{64+36}=\sqrt{100}=10", font_size=30),
        ...
        block.next_to(fig, RIGHT, buff=0.4)
    ...
    def part_trial_samples(self):
        ...
        calc = [
            MathTex(r"AP=\sqrt{4^2+6^2}=\sqrt{16+36}=\sqrt{52}", font_size=30),
            MathTex(r"\sqrt{52}=\sqrt{4\cdot 13}=2\sqrt{13}\approx 7.211", font_size=30),
            MathTex(r"PB=\sqrt{4^2+2^2}=\sqrt{16+4}=\sqrt{20}", font_size=30),
            MathTex(r"\sqrt{20}=\sqrt{4\cdot 5}=2\sqrt{5}\approx 4.472", font_size=30),
            MathTex(r"7.211+4.472=11.683", font_size=34, color=YELLOW),
        ]
        ...
        table = self.aligned_table(
            [
                [
                    self._line(MathTex(r"P", font_size=22, color=GREY_B), "の横", MathTex(r"x", font_size=22, color=GREY_B), font_size=16, color=GREY_B),
                ...
                [MathTex(r"4", font_size=26, color=YELLOW), MathTex(r"7.211", font_size=26, color=YELLOW), MathTex(r"4.472", font_size=26, color=YELLOW), MathTex(r"11.683", font_size=26, color=YELLOW)],
        ...
        notes = [
            ...
            self._line(MathTex(r"4", font_size=26), "と", MathTex(r"8", font_size=26), "のあいだは、まだ調べていません。", font_size=22),
        ]
        shown = VGroup()
        for i, mob in enumerate(notes):
            if i == 0:
    ...
    def part_step1_length(self):
        ...
        lead = self._line("触れる点を", MathTex(r"P=(x,0)", font_size=28), "とおいて、長さを式にします。", font_size=24)
        ...
        notes = [
            self._line("このまま", MathTex(r"x", font_size=26), "で微分してもよいのですが、計算が重いです。", font_size=22),
    ...
    def part_step2_reflect(self):
        ...
        lead = self._line(MathTex(r"B", font_size=28), "を、川に対して反対側へ折り返します。", font_size=24)
        self.below_chip(lead, chip, buff=0.24)
        self._fit_left(lead)
        self.play(FadeIn(lead), run_time=0.45)
        self.linger("B を、川に対して反対側へ折り返します。")

        fig = self._base_figure(p_x=4.0, show_b_prime=False)
        prime = VGroup(*self._b_prime_layer(p_x=4.0))
        full = VGroup(fig, prime)
        self.stack_below(full, lead, buff=0.22)
        full.to_edge(LEFT, buff=0.45)
        self.play(FadeIn(fig), run_time=0.6)
        self.play(FadeIn(prime), run_time=0.8)
        self.linger(3.2)

        rows = [
            self._line(MathTex(r"H", font_size=26), "は", MathTex(r"BB'", font_size=26), "の中点で、川の上にある", font_size=20),
            MathTex(r"BH=B'H", font_size=28),
            self._line("角", MathTex(r"BHP", font_size=24), "と角", MathTex(r"B'HP", font_size=24), "は直角。", MathTex(r"HP", font_size=24), "は共通", font_size=20),
            self._line("直角三角形", MathTex(r"BHP", font_size=24), "と", MathTex(r"B'HP", font_size=24), "は合同", font_size=20),
            MathTex(r"BP=B'P", font_size=36, color=YELLOW),
        ]
        block = VGroup(*rows).arrange(DOWN, buff=0.14, aligned_edge=LEFT)
        block.next_to(full, RIGHT, buff=0.35)
        block.align_to(full, UP)
        self._fit(block, 7.6)
        for row in rows:
            self.play(FadeIn(row), run_time=0.45)
            self.linger(3.2)
        ...
        notes = [
            self._line("どの", MathTex(r"P", font_size=24), "でも、川の上なら", MathTex(r"BP=B'P", font_size=24), "です。", font_size=20),
            self._line("だから", MathTex(r"AP+PB", font_size=24), "は", MathTex(r"AP+PB'", font_size=24), "と同じ長さです。", font_size=20),
        ]
        shown = VGroup()
        for i, mob in enumerate(notes):
            if i == 0:
    ...
    def part_step3_straight(self):
        ...
        lead = self._line(MathTex(r"A", font_size=28), "から", MathTex(r"B'", font_size=28), "へ行く道は、直線がいちばん短いです。", font_size=24)
        self.below_chip(lead, chip, buff=0.24)
        self._fit_left(lead)
        self.play(FadeIn(lead), run_time=0.45)
        ...
        rows = [
            self._line("任意の", MathTex(r"P", font_size=26), "に対して", MathTex(r"AP+PB'", font_size=26), "は折れ線", font_size=20),
            self.ja_text("二点を結ぶ最短は直線", font_size=20),
            MathTex(r"AP+PB'\ge AB'", font_size=32, color=YELLOW),
            self._line("等号は、", MathTex(r"P", font_size=24), "が線分", MathTex(r"AB'", font_size=24), "の上にあるとき", font_size=20),
            self._line(MathTex(r"P", font_size=24), "は川の上にもいなければならない", font_size=20),
            self._line("最短の", MathTex(r"P", font_size=24), "は、直線", MathTex(r"AB'", font_size=24), "と川の交点", font_size=20),
    ...
    def part_step4_angles(self):
        ...
        fig = self._base_figure(p_x=6.0, show_b_prime=True, show_straight=True, show_angles=True)
        self.stack_below(fig, lead, buff=0.18)
        fig.set_x(-1.6)
        overflow = (-config.frame_width / 2 + 0.08) - fig.get_left()[0]
        if overflow > 0:
            fig.shift(RIGHT * overflow)
        self.play(FadeIn(fig), run_time=0.8)
        self.linger(3.2)

        rows = [
            self._line(MathTex(r"A", font_size=26), "、", MathTex(r"P_*", font_size=26), "、", MathTex(r"B'", font_size=26), "は一直線", font_size=20),
            self.ja_text("法線とこの直線がつくる対頂角は等しい", font_size=20),
            self._line("折り返しで", MathTex(r"B'P_*", font_size=24), "の角は", MathTex(r"BP_*", font_size=24), "の角に等しい", font_size=20),
            MathTex(r"\theta=\theta", font_size=36, color=YELLOW),
        ]
        block = VGroup(*rows).arrange(DOWN, buff=0.14, aligned_edge=LEFT)
        block.next_to(fig, RIGHT, buff=0.4)
        block.align_to(fig, UP)
    ...
    def part_example(self):
        ...
        second = [
            ...
            self._line(MathTex(r"x=4", font_size=30), "の合計", MathTex(r"11.683", font_size=30), font_size=22),
        ...
        table = self.aligned_table(
            [
                ...
                [MathTex(r"6", font_size=28, color=GREEN), MathTex(r"11.314", font_size=28, color=GREEN)],
        ...
        notes = [
            self._line("試行で残っていたあいだに、交点", MathTex(r"x=6", font_size=26), "がありました。", font_size=22),
            self.ja_text("B の真下でも、まん中でもなく、直線 AB' が川を切る場所です。", font_size=22),
        ]
        shown = VGroup()
        for i, mob in enumerate(notes):
            if i == 0:
                self.stack_below(mob, table, buff=0.2)
            else:
    ...
    def part_generalize(self):
        ...
        eq = self.ja_text("等号は、P が AB' と境界の交点のときです。", font_size=22)
        self.stack_below(eq, ineq, buff=0.24)
        eq.set_x(0)
        self.play(FadeIn(eq), run_time=0.4)
        self.linger(eq.text, extra=0.3)
    ...
    def part_summary(self):
        ...
        related = self.ja_text("速さが途中で変わる道なら、角は等しくならず、折れ方が変わります。", font_size=22, color=GREY_B)
    ...
    def _dot(self, wx, wy, color=WHITE):
        d = Dot(self._pt(wx, wy), color=color, radius=0.07)
        return d

    def _lab(self, tex, wx, wy, color=WHITE, buff_dir=UR, font_size=26):
        m = MathTex(tex, font_size=font_size, color=color)
        m.next_to(self._pt(wx, wy), buff_dir, buff=0.12)
        return m

    def _base_figure(self, p_x, show_b_prime=False, show_straight=False, show_angles=False, path_opacity=1.0):
        a = self._pt(self.AX, self.AY)
        b = self._pt(self.BX, self.BY)
        p = self._pt(p_x, 0.0)
        bp = self._pt(self.BX, -self.BY)
        h = self._pt(self.BX, 0.0)

        river = Line(self._pt(-0.6, 0.0), self._pt(8.6, 0.0), color=BLUE, stroke_width=5)
        river_lab = self.ja_text("川", font_size=20, color=BLUE)
        river_lab.next_to(river, RIGHT, buff=0.12)

        a_dot = Dot(a, color=YELLOW, radius=0.08)
        b_dot = Dot(b, color=TEAL, radius=0.08)
        p_dot = Dot(p, color=ORANGE, radius=0.08)
        a_lab = MathTex(r"A", font_size=28, color=YELLOW).next_to(a_dot, UL, buff=0.1)
        b_lab = MathTex(r"B", font_size=28, color=TEAL).next_to(b_dot, UR, buff=0.1)
        p_lab = MathTex(r"P", font_size=28, color=ORANGE).next_to(p_dot, DOWN, buff=0.12)
    ...
    def _b_prime_layer(self, p_x, include_path=True, show_straight=False):
        ...
        h_lab = MathTex(r"H", font_size=24, color=GREY_B).next_to(h_dot, DR, buff=0.08)
        bp_dot = Dot(bp, color=PURPLE, radius=0.08)
        bp_lab = MathTex(r"B'", font_size=28, color=PURPLE).next_to(bp_dot, DR, buff=0.1)
        items = [bb, h_dot, h_lab, bp_dot, bp_lab]
        if include_path:
            pb_dash = DashedLine(p, bp, color=PURPLE, stroke_width=2.5)
            items.append(pb_dash)
        if show_straight:
            abp = DashedLine(a, bp, color=WHITE, stroke_width=2)
            items.insert(0, abp)
        return items
    ...
    def _angle_layer(self, p_x):
        ...
        b = self._pt(self.BX, self.BY)
        p = self._pt(p_x, 0.0)
        n_line = DashedLine(p + DOWN * 0.55, p + UP * 1.15, color=GREY_A, stroke_width=2)
        ap = Line(p, a, color=YELLOW)
        pb = Line(p, b, color=TEAL)
        n_up = Line(p, p + UP * 1.15, color=GREY_A)
        try:
            ang_i = Angle(n_up, ap, radius=0.38, color=YELLOW)
            ang_r = Angle(n_up, pb, radius=0.52, color=TEAL)
        except Exception:
            ang_i = Dot(p, radius=0.01)
            ang_r = Dot(p, radius=0.01)
        th1 = MathTex(r"\theta", font_size=24, color=YELLOW)
        th2 = MathTex(r"\theta", font_size=24, color=TEAL)
        th1.move_to(p + 0.55 * UP + 0.45 * LEFT)
        th2.move_to(p + 0.55 * UP + 0.55 * RIGHT)
        n_lab = self.ja_text("法線", font_size=16, color=GREY_A)
        n_lab.next_to(n_line, RIGHT, buff=0.08)
        n_lab.align_to(n_line, UP)
        return [n_line, ang_i, ang_r, th1, th2, n_lab]
