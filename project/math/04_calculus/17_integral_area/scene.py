...
class IntegralArea(JapaneseScene):
    ...
    def draw_curve(self):
        ...
        self.graph = self.axes.plot(self.f, x_range=[0.3, 4.4], color=BLUE, stroke_width=4)
    ...
    def _rects(self, n, color=GREEN):
        ...
        for i in range(n):
            ...
            p11 = self.axes.c2p(x + dx, self.f(x))
            p01 = self.axes.c2p(x, self.f(x))
    ...
    def show_formula(self):
        formula = MathTex(r"\int_a^b f(x)\,dx=\text{面積}").scale(0.95)
        # Japanese in MathTex is risky
        formula = VGroup(
