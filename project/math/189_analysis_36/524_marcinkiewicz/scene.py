...
class Marcinkiewicz(PacedScene):
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq = MathTex(r"T:L^{p_i}\to L^{q_i,\infty}\ \Rightarrow\ T:L^{p_\theta}\to L^{q_\theta}").scale(0.62)
        ...
        self.read(0.2)
        eq2 = MathTex(r"T:L^{p_i}\to L^{q_i,\infty}\ \Rightarrow\ T:L^{p_\theta}\to L^{q_\theta}").scale(0.62)
        eq2.move_to(eq)
