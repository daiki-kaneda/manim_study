...
class AdamW(PacedScene):
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq = MathTex(r"\theta\leftarrow\theta-\eta(\hat m/\sqrt{\hat v}+\lambda\theta)").scale(0.62)
        ...
        self.read(0.2)
        eq2 = MathTex(r"\theta\leftarrow\theta-\eta(\hat m/\sqrt{\hat v}+\lambda\theta)").scale(0.62)
        eq2.move_to(eq)
