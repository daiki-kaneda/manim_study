...
class Muon(PacedScene):
    """#778 Muon（約45秒）"""

    def construct(self):
        self.show_heading("Muon")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("直交化した更新", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("勾配を直交化", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.9)
        self.read(0.25)
        cap2 = self.ja_text("深層学習向け", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq = MathTex("U"+chr(92)+"leftarrow"+chr(92)+"mathrm"+"{Orth}(G)").scale(0.62)
        ...
        self.read(0.2)
        eq2 = MathTex("U"+chr(92)+"leftarrow"+chr(92)+"mathrm"+"{Orth}(G)").scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex("U"+chr(92)+"leftarrow"+chr(92)+"mathrm"+"{Orth}(G)").scale(0.75)
