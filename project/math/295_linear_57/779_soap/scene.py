...
class SOAP(PacedScene):
    """#779 SOAP（約45秒）"""

    def construct(self):
        self.show_heading("SOAP")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("シャムプー系の最適化", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("クロネッカー構造", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.9)
        self.read(0.25)
        cap2 = self.ja_text("二次情報を近似", font_size=24).move_to(self.note)
    ...
    def show_formula(self):
        formula = MathTex("P"+chr(92)+"approx"+" L"+chr(92)+"otimes"+" R").scale(0.75)
