...
class DeLongchampsPoint(PacedScene):
    """#782 デロングシャン点（約45秒）"""

    def construct(self):
        self.show_heading("デロングシャン点")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("垂心の等角共役", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("垂心三角形", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.9)
        self.read(0.25)
        cap2 = self.ja_text("外心の鏡像", font_size=24).move_to(self.note)
    ...
    def show_formula(self):
        formula = MathTex("L=X(20)").scale(0.75)
