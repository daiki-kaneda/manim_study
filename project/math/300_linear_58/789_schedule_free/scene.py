...
class ScheduleFree(PacedScene):
    """#789 Schedule-Free（約45秒）"""

    def construct(self):
        self.show_heading("Schedule-Free")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("学習率表なし最適化", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("平均化で収束", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.9)
        self.read(0.25)
        cap2 = self.ja_text("スケジュール不要", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq2 = MathTex("z_{t+1}=z_t-"+chr(92)+"gamma"+chr(92)+"nabla"+" f(x_t)").scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex("z_{t+1}=z_t-"+chr(92)+"gamma"+chr(92)+"nabla"+" f(x_t)").scale(0.75)
