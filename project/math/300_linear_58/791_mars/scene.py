...
class MARS(PacedScene):
    """#791 MARS（約45秒）"""

    def construct(self):
        self.show_heading("MARS")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("分散縮約つき更新", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("補正勾配", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.9)
        self.read(0.25)
        cap2 = self.ja_text("大規模向け", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq2 = MathTex("c_t=g_t+"+chr(92)+"gamma"+"(g_t-g_{t-1})").scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex("c_t=g_t+"+chr(92)+"gamma"+"(g_t-g_{t-1})").scale(0.75)
