...
class TalbotPoint(PacedScene):
    """#792 タルボット点（約45秒）"""

    def construct(self):
        self.show_heading("タルボット点")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("自己等角共役点", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("シムソン線族", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.9)
        self.read(0.25)
        cap2 = self.ja_text("三角形の中心", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq2 = MathTex("Ta=X(25)").scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex("Ta=X(25)").scale(0.75)
