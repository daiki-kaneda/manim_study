...
class NAdam(PacedScene):
    """#801 NAdam（約45秒）"""

    def construct(self):
        self.show_heading("NAdam")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("ネステロフつきAdam", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("運動量を先読み", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.9)
        self.read(0.25)
        cap2 = self.ja_text("適応学習率", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq2 = MathTex("m_t="+chr(92)+"beta"+"_1 m_{t-1}+(1-"+chr(92)+"beta"+"_1)g_t").scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex("m_t="+chr(92)+"beta"+"_1 m_{t-1}+(1-"+chr(92)+"beta"+"_1)g_t").scale(0.72)
