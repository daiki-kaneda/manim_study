...
class Adamax(PacedScene):
    """#802 Adamax（約45秒）"""

    def construct(self):
        self.show_heading("Adamax")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("無限ノルム適応法", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("maxで二次モーメント", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.9)
        self.read(0.25)
        cap2 = self.ja_text("Adamの安定版", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq2 = MathTex("u_t="+chr(92)+"max"+"("+chr(92)+"beta"+"_2 u_{t-1},|g_t|)").scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex("u_t="+chr(92)+"max"+"("+chr(92)+"beta"+"_2 u_{t-1},|g_t|)").scale(0.72)
