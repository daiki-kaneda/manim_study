...
class Kron(PacedScene):
    """#813 Kron（約45秒）"""

    def construct(self):
        self.show_heading("Kron")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("クロネッカー因子分解", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("二次情報の近似", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.9)
        self.read(0.25)
        cap2 = self.ja_text("Shampoo系", font_size=24).move_to(self.note)
    ...
    def show_formula(self):
        formula = MathTex("P"+chr(92)+"approx"+" L"+chr(92)+"otimes"+" R").scale(0.72)
