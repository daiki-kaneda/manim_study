...
class SecondNapoleon(PacedScene):
    """#816 第二ナポレオン（約45秒）"""

    def construct(self):
        self.show_heading("第二ナポレオン")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("内側ナポレオン中心", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("内側に正三角形", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.9)
        self.read(0.25)
        cap2 = self.ja_text("第一と共役", font_size=24).move_to(self.note)
    ...
    def show_formula(self):
        formula = MathTex("N=X(18)").scale(0.72)
