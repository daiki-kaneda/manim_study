...
class KiepertPoint(PacedScene):
    """#817 キーペルト点（約45秒）"""

    def construct(self):
        self.show_heading("キーペルト点")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("キーペルト双曲線上", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("等脚三角形", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.9)
        self.read(0.25)
        cap2 = self.ja_text("三角形中心", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq2 = MathTex("K=X(115)").scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex("K=X(115)").scale(0.72)
