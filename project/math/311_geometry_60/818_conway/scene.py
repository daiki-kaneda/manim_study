...
class ConwayPoint(PacedScene):
    """#818 コンウェイ点（約45秒）"""

    def construct(self):
        self.show_heading("コンウェイ点")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("コンウェイの中心", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("拡張辺の交点", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.9)
        self.read(0.25)
        cap2 = self.ja_text("三角形幾何", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq2 = MathTex("Co=X(37)").scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex("Co=X(37)").scale(0.72)
