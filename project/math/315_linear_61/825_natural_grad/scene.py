...
class NaturalGrad(PacedScene):
    """#825 NaturalGrad（約45秒）"""

    def construct(self):
        self.show_heading("NaturalGrad")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("フィッシャー計量で進む", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("情報幾何の勾配", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.9)
        self.read(0.25)
        cap2 = self.ja_text("二次近似", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq = MathTex("F^{-1}"+chr(92)+"nabla"+" L").scale(0.62)
        ...
        self.read(0.2)
        eq2 = MathTex("F^{-1}"+chr(92)+"nabla"+" L").scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex("F^{-1}"+chr(92)+"nabla"+" L").scale(0.72)
