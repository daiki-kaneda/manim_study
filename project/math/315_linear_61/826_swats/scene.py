...
class SWATS(PacedScene):
    """#826 SWATS（約45秒）"""

    def construct(self):
        self.show_heading("SWATS")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("AdamからSGDへ切替", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("適応→通常", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.9)
        self.read(0.25)
        cap2 = self.ja_text("学習後半の安定", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq = MathTex("Adam"+chr(92)+"to"+" SGD").scale(0.62)
        ...
        self.read(0.2)
        eq2 = MathTex("Adam"+chr(92)+"to"+" SGD").scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex("Adam"+chr(92)+"to"+" SGD").scale(0.72)
