...
class SchifflerPoint(PacedScene):
    """#828 シュライフラ点（約45秒）"""

    def construct(self):
        self.show_heading("シュライフラ点")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("内心三角形のオイラー", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("内心と頂点", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.9)
        self.read(0.25)
        cap2 = self.ja_text("共線条件", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq2 = MathTex("Sc=X(21)").scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex("Sc=X(21)").scale(0.72)
