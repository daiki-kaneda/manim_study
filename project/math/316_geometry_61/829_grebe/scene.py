...
class GrebePoint(PacedScene):
    """#829 グレブ点（約45秒）"""

    def construct(self):
        self.show_heading("グレブ点")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("接線三角形の中心", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("外接正方形系", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.9)
        self.read(0.25)
        cap2 = self.ja_text("シュピーカー近傍", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq2 = MathTex("Gr=X(112)").scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex("Gr=X(112)").scale(0.72)
