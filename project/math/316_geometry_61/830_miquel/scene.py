...
class MiquelPoint(PacedScene):
    """#830 ミッケル点（約45秒）"""

    def construct(self):
        self.show_heading("ミッケル点")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("円の三交点", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("辺上の三点", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.9)
        self.read(0.25)
        cap2 = self.ja_text("円が一点で会す", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq2 = MathTex("M=X(111)").scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex("M=X(111)").scale(0.72)
