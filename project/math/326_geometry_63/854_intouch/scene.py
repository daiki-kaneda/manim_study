...
class IntouchTriangle(PacedScene):
    ...
    def draw(self):
        ...
        note = self.ja_text("接点が作る三角形", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("内接円の接点", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.9)
        self.read(0.25)
        cap2 = self.ja_text("ジェルゴンヌへ", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq2 = MathTex(chr(92)+"triangle"+"_{intouch}").scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex(chr(92)+"triangle"+"_{intouch}").scale(0.70)
