...
class AggMo(PacedScene):
    """#863 AggMo（約45秒）"""

    def construct(self):
        self.show_heading("AggMo")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("複数动量の集約", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("速度の平均", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq2 = MathTex("v="+chr(92)+"sum"+"_i"+chr(92)+"alpha"+"_i v^{(i)}").scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex("v="+chr(92)+"sum"+"_i"+chr(92)+"alpha"+"_i v^{(i)}").scale(0.68)
