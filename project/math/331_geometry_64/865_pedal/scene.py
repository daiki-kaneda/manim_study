...
class PedalTriangle(PacedScene):
    """#865 ペダル三角形（約45秒）"""

    def construct(self):
        self.show_heading("ペダル三角形")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("垂線の足の三角形", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("点からの垂足", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.9)
        self.read(0.25)
        cap2 = self.ja_text("シムソンの一般化", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq2 = MathTex(chr(92)+"triangle"+"_{pedal}(P)").scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex(chr(92)+"triangle"+"_{pedal}(P)").scale(0.68)
