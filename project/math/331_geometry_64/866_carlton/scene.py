...
class CarltonPoint(PacedScene):
    """#866 カールトン点（約45秒）"""

    def construct(self):
        self.show_heading("カールトン点")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("三角形中心の一例", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("近年の中心", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq2 = MathTex("Ca=X(165)").scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex("Ca=X(165)").scale(0.68)
