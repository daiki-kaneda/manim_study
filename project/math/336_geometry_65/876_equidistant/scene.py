...
class EquidistantPoint(PacedScene):
    """#876 等距離点（約45秒）"""

    def construct(self):
        self.show_heading("等距離点")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("二点から等距離", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("垂直二等分線", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.9)
        self.read(0.25)
        cap2 = self.ja_text("外心の一般化", font_size=24).move_to(self.note)
    ...
    def show_formula(self):
        formula = MathTex("|PA|=|PB|").scale(0.68)
