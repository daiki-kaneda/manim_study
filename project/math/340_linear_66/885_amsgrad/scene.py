...
class AMSGrad(PacedScene):
    """#885 AMSGrad（約45秒）"""

    def construct(self):
        self.show_heading("AMSGrad")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("最大二次モーメント", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("Adamの収束修正", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.9)
        self.read(0.25)
        cap2 = self.ja_text("非増加v", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq2 = MathTex(chr(92)+"hat"+" v_t="+chr(92)+"max"+"("+chr(92)+"hat"+" v_{t-1},v_t)").scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex(chr(92)+"hat"+" v_t="+chr(92)+"max"+"("+chr(92)+"hat"+" v_{t-1},v_t)").scale(0.68)
