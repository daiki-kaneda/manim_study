...
class Eccentric(PacedScene):
    """#889 エクセントリック（約45秒）"""

    def construct(self):
        self.show_heading("エクセントリック")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("離心率の幾何", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("焦点と準線", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.9)
        self.read(0.25)
        cap2 = self.ja_text("円錐曲線", font_size=24).move_to(self.note)
    ...
    def show_formula(self):
        formula = MathTex("e=c/a").scale(0.68)
