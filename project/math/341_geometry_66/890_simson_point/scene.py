...
class SimsonPoint(PacedScene):
    """#890 シムソン線点（約45秒）"""

    def construct(self):
        self.show_heading("シムソン線点")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("シムソン線上の点", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("外接円上から", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.9)
        self.read(0.25)
        cap2 = self.ja_text("共線の足", font_size=24).move_to(self.note)
    ...
    def show_formula(self):
        formula = MathTex("S(P)").scale(0.68)
