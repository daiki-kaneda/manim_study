...
class AdaShift(PacedScene):
    """#897 AdaShift（約45秒）"""

    def construct(self):
        self.show_heading("AdaShift")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("時刻ずらし適応法", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("非同期更新向け", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.9)
        self.read(0.25)
        cap2 = self.ja_text("Adamの変形", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq2 = MathTex("m_t="+chr(92)+"beta"+" m_{t-n}+(1-"+chr(92)+"beta"+")g_t").scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex("m_t="+chr(92)+"beta"+" m_{t-n}+(1-"+chr(92)+"beta"+")g_t").scale(0.68)
