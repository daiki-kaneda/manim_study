...
class Eve(PacedScene):
    """#898 Eve（約45秒）"""

    def construct(self):
        self.show_heading("Eve")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("相対変化で適応", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("目的関数の変動", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.9)
        self.read(0.25)
        cap2 = self.ja_text("Adam系", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq2 = MathTex("d_t=|f_t-f_{t-1}|/|f_{t-1}|").scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex("d_t=|f_t-f_{t-1}|/|f_{t-1}|").scale(0.68)
