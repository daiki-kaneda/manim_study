...
class ThreeCollinear(PacedScene):
    """#913 三点共線（約45秒）"""

    def construct(self):
        self.show_heading("三点共線")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("一直線上の三点", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("面積が零", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.9)
        self.read(0.25)
        cap2 = self.ja_text("メネラウスへ", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq2 = MathTex(chr(92)+"det"+"(A,B,C)=0").scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex(chr(92)+"det"+"(A,B,C)=0").scale(0.68)
