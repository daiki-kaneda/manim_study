...
class AdaX(PacedScene):
    """#921 AdaX（約45秒）"""

    def construct(self):
        self.show_heading("AdaX")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("加速つき適応法", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("ネステロフ＋Adam", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.9)
        self.read(0.25)
        cap2 = self.ja_text("収束改善", font_size=24).move_to(self.note)
    ...
    def show_formula(self):
        formula = MathTex("m_t="+chr(92)+"beta"+"_1 m_{t-1}+(1-"+chr(92)+"beta"+"_1)g_t").scale(0.68)
