...
class PowerSign(PacedScene):
    """#922 PowerSign（約45秒）"""

    def construct(self):
        self.show_heading("PowerSign")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("符号つき冪更新", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("相対的な大きさ", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.9)
        self.read(0.25)
        cap2 = self.ja_text("ニューラル向け", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq2 = MathTex(chr(92)+"Delta"+" x"+chr(92)+"propto"+"-"+chr(92)+"mathrm"+"{sign}(g)|g|^p").scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex(chr(92)+"Delta"+" x"+chr(92)+"propto"+"-"+chr(92)+"mathrm"+"{sign}(g)|g|^p").scale(0.68)
