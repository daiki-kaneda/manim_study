...
class LaProp(PacedScene):
    """#934 LaProp（約45秒）"""

    def construct(self):
        self.show_heading("LaProp")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("層適応の伝播", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("学習率の層間", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.9)
        self.read(0.25)
        cap2 = self.ja_text("深層向け", font_size=24).move_to(self.note)
