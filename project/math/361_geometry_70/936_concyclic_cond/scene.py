...
class ConcyclicConditionPoint(PacedScene):
    """#936 共円条件点（約45秒）"""

    def construct(self):
        self.show_heading("共円条件点")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("共円を判定する点", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("角度条件", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.9)
        self.read(0.25)
        cap2 = self.ja_text("方べき", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq2 = MathTex(chr(92)+"angle"+" APB="+chr(92)+"angle"+" AQB").scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex(chr(92)+"angle"+" APB="+chr(92)+"angle"+" AQB").scale(0.68)
