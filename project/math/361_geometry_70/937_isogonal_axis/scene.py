...
class IsogonalAxis(PacedScene):
    """#937 等角共役軸（約45秒）"""

    def construct(self):
        self.show_heading("等角共役軸")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("等角共役の軸", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("内心を通る", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.9)
        self.read(0.25)
        cap2 = self.ja_text("対称の線", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq2 = MathTex(chr(92)+"ell"+"_{iso}").scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex(chr(92)+"ell"+"_{iso}").scale(0.68)
