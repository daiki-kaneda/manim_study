...
class SimsonFamily(PacedScene):
    """#938 シムソン族（約45秒）"""

    def construct(self):
        self.show_heading("シムソン族")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("シムソン線の族", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("外接円を動く", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.9)
        self.read(0.25)
        cap2 = self.ja_text("包絡線", font_size=24).move_to(self.note)
