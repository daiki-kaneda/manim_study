...
class AdaPlus(PacedScene):
    """#946 AdaPlus（約45秒）"""

    def construct(self):
        self.show_heading("AdaPlus")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("Adamの改良版", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("バイアス補正強化", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.9)
        self.read(0.25)
        cap2 = self.ja_text("安定な適応", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq2 = MathTex(chr(92)+"hat"+" m_t=m_t/(1-"+chr(92)+"beta"+"_1^t)").scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex(chr(92)+"hat"+" m_t=m_t/(1-"+chr(92)+"beta"+"_1^t)").scale(0.68)
