...
class IncenterReflection(PacedScene):
    """#948 内心反射（約45秒）"""

    def construct(self):
        self.show_heading("内心反射")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("内心に関する反射", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("角の二等分線", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.9)
        self.read(0.25)
        cap2 = self.ja_text("等角性", font_size=24).move_to(self.note)
    ...
    def show_formula(self):
        formula = MathTex("R_I").scale(0.68)
