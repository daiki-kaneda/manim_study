...
class EveW(PacedScene):
    """#959 EveW（約45秒）"""

    def construct(self):
        self.show_heading("EveW")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("Eve＋重み減衰", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("相対変化適応", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.9)
        self.read(0.25)
        cap2 = self.ja_text("汎化向け", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq2 = MathTex("d_t=|"+chr(92)+"Delta"+" f|/|f|").scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex("d_t=|"+chr(92)+"Delta"+" f|/|f|").scale(0.68)
