...
class ExcircleCenterCircle(PacedScene):
    """#961 傍心円（約45秒）"""

    def construct(self):
        self.show_heading("傍心円")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("傍接円の言い換え", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("傍心が中心", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.9)
        self.read(0.25)
        cap2 = self.ja_text("接線長", font_size=24).move_to(self.note)
    ...
    def show_formula(self):
        formula = MathTex("I_a").scale(0.68)
