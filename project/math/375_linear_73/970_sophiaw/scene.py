...
class SophiaW(PacedScene):
    """#970 SophiaW（約45秒）"""

    def construct(self):
        self.show_heading("SophiaW")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("Sophia＋減衰", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("ヘッセ対角推定", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.9)
        self.read(0.25)
        cap2 = self.ja_text("クリップ更新", font_size=24).move_to(self.note)
