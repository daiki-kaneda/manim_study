...
class TangentLengths(PacedScene):
    ...
    def draw(self):
        ...
        note = self.ja_text("接点までの長さ", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("等しい接線", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.9)
        self.read(0.25)
        cap2 = self.ja_text("半周長表示", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq2 = MathTex("t_a=s-a").scale(0.62)
        eq2.move_to(eq)
