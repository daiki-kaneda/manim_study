...
class Semiperimeter(PacedScene):
    """#973 半周長公式（約45秒）"""

    def construct(self):
        self.show_heading("半周長公式")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("周長の半分", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("ヘロンの入口", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.9)
        self.read(0.25)
        cap2 = self.ja_text("接線長の和", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq2 = MathTex("s=(a+b+c)/2").scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex("s=(a+b+c)/2").scale(0.68)
