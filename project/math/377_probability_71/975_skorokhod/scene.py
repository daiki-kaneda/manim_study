...
class Skorokhod(PacedScene):
    ...
    def draw(self):
        ...
        note = self.ja_text("反射の方程式", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("押し上げ過程", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.9)
        self.read(0.25)
        cap2 = self.ja_text("局所時間", font_size=24).move_to(self.note)
    ...
    def show_formula(self):
        formula = MathTex("X=Y+L").scale(0.68)
