...
class AdaFactorW(PacedScene):
    """#995 AdaFactorW（約45秒）"""

    def construct(self):
        self.show_heading("AdaFactorW")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("AdaFactor＋減衰", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("因子化二次モーメント", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq = MathTex("R_{ij}"+chr(92)+"approx"+" r_i c_j").scale(0.62)
        ...
        self.read(0.2)
        eq2 = MathTex("R_{ij}"+chr(92)+"approx"+" r_i c_j").scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex("R_{ij}"+chr(92)+"approx"+" r_i c_j").scale(0.68)
