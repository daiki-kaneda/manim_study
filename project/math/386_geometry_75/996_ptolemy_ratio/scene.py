...
class PtolemyRatio(PacedScene):
    """#996 トレミー比（約45秒）"""

    def construct(self):
        self.show_heading("トレミー比")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("円内接四辺形", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("積の和", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.9)
        self.read(0.25)
        cap2 = self.ja_text("相似比", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq2 = MathTex("ac+bd=pq").scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex("ac+bd=pq").scale(0.68)
