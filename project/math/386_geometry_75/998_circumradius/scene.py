...
class CircumradiusFormula(PacedScene):
    """#998 外接半径公式（約45秒）"""

    def construct(self):
        self.show_heading("外接半径公式")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("外接円の半径", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("辺と面積", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.9)
        self.read(0.25)
        cap2 = self.ja_text("正弦法則", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq2 = MathTex("R=abc/(4K)").scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex("R=abc/(4K)").scale(0.68)
