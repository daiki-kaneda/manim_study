...
class PadamW(PacedScene):
    """#1005 PadamW（約45秒）"""

    def construct(self):
        self.show_heading("PadamW")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("Padam＋重み減衰", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("p次根スケール", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq = MathTex("x"+chr(92)+"leftarrow"+" x-"+chr(92)+"eta"+" m/("+chr(92)+"hat"+" v)^{p}").scale(0.62)
        ...
        self.read(0.2)
        eq2 = MathTex("x"+chr(92)+"leftarrow"+" x-"+chr(92)+"eta"+" m/("+chr(92)+"hat"+" v)^{p}").scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex("x"+chr(92)+"leftarrow"+" x-"+chr(92)+"eta"+" m/("+chr(92)+"hat"+" v)^{p}").scale(0.68)
