...
class ShampooW(PacedScene):
    """#1007 ShampooW（約45秒）"""

    def construct(self):
        self.show_heading("ShampooW")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def mid(self):
        cap = self.ja_text("行列前処理", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq2 = MathTex("P^{-1/2}g").scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex("P^{-1/2}g").scale(0.68)
