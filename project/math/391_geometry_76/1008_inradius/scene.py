...
class InradiusFormula(PacedScene):
    """#1008 内心半径公式（約45秒）"""

    def construct(self):
        self.show_heading("内心半径公式")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("内接円の半径", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("面積と半周長", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.9)
        self.read(0.25)
        cap2 = self.ja_text("r=K/s", font_size=24).move_to(self.note)
