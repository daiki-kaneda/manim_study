...
class ApolloW(PacedScene):
    """#1030 ApolloW（約45秒）"""

    def construct(self):
        self.show_heading("ApolloW")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("Apollo＋減衰", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("準ニュートン風", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq = MathTex("B"+chr(92)+"leftarrow"+" B+uu^T").scale(0.62)
        ...
        self.read(0.2)
        eq2 = MathTex("B"+chr(92)+"leftarrow"+" B+uu^T").scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex("B"+chr(92)+"leftarrow"+" B+uu^T").scale(0.68)
