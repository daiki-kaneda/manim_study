...
class ExradiusShort(PacedScene):
    """#1032 傍接半径（約45秒）"""

    def construct(self):
        self.show_heading("傍接半径")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("傍接円半径の別称", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("面積表示", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.9)
        self.read(0.25)
        cap2 = self.ja_text("s-a", font_size=24).move_to(self.note)
