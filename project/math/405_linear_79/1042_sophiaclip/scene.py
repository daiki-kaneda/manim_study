...
class SophiaClip(PacedScene):
    """#1042 SophiaClip（約45秒）"""

    def construct(self):
        self.show_heading("SophiaClip")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("クリップつきSophia", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("ヘッセ比を制限", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq2 = MathTex(chr(92)+"Delta"+"="+chr(92)+"mathrm"+"{clip}(m/"+chr(92)+"hat"+" h)").scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex(chr(92)+"Delta"+"="+chr(92)+"mathrm"+"{clip}(m/"+chr(92)+"hat"+" h)").scale(0.68)
