...
class SideAreaRatio(PacedScene):
    """#1044 三辺面積比（約45秒）"""

    def construct(self):
        self.show_heading("三辺面積比")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("辺と面積の比", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("高さで結ぶ", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq2 = MathTex("K:K'=a:a'").scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex("K:K'=a:a'").scale(0.68)
