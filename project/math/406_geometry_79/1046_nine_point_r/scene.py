...
class NinePointRadius(PacedScene):
    """#1046 九点円半径（約45秒）"""

    def construct(self):
        self.show_heading("九点円半径")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("九点円の半径", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("外接半径の半分", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq2 = MathTex("R_9=R/2").scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex("R_9=R/2").scale(0.68)
