...
class ShampooClip(PacedScene):
    """#1054 ShampooClip（約45秒）"""

    def construct(self):
        self.show_heading("ShampooClip")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("クリップつきShampoo", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("行列根を制限", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq = MathTex("P^{-1/2}"+chr(92)+"leftarrow"+chr(92)+"mathrm"+"{clip}(P^{-1/2})").scale(0.62)
        ...
        self.read(0.2)
        eq2 = MathTex("P^{-1/2}"+chr(92)+"leftarrow"+chr(92)+"mathrm"+"{clip}(P^{-1/2})").scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex("P^{-1/2}"+chr(92)+"leftarrow"+chr(92)+"mathrm"+"{clip}(P^{-1/2})").scale(0.68)
