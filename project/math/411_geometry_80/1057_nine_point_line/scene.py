...
class NinePointLine(PacedScene):
    """#1057 九点円中心線（約45秒）"""

    def construct(self):
        self.show_heading("九点円中心線")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("九点円中心の位置", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("OHの中点", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.9)
        self.read(0.25)
        cap2 = self.ja_text("オイラー線", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq2 = MathTex("N=(O+H)/2").scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex("N=(O+H)/2").scale(0.68)
