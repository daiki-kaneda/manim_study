...
class KronClip(PacedScene):
    """#1065 KronClip（約45秒）"""

    def construct(self):
        self.show_heading("KronClip")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("クリップつきKron", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("因子制限", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.9)
        self.read(0.25)
        cap2 = self.ja_text("安定前処理", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq = MathTex("L,R"+chr(92)+"leftarrow"+chr(92)+"mathrm"+"{clip}(L,R)").scale(0.62)
        ...
        self.read(0.2)
        eq2 = MathTex("L,R"+chr(92)+"leftarrow"+chr(92)+"mathrm"+"{clip}(L,R)").scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex("L,R"+chr(92)+"leftarrow"+chr(92)+"mathrm"+"{clip}(L,R)").scale(0.68)
