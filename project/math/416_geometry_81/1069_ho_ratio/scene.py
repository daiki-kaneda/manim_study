...
class HORatio(PacedScene):
    """#1069 垂心外心比（約45秒）"""

    def construct(self):
        self.show_heading("垂心外心比")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("HとOの位置関係", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("重心が内分", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.9)
        self.read(0.25)
        cap2 = self.ja_text("1:2", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq2 = MathTex("HG:GO=2:1").scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex("HG:GO=2:1").scale(0.68)
