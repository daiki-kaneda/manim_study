...
class SophiaSoft(PacedScene):
    """#1079 SophiaSoft（約45秒）"""

    def construct(self):
        self.show_heading("SophiaSoft")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("軟化Sophia", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("緩和ヘッセ比", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq2 = MathTex(chr(92)+"Delta"+"="+chr(92)+"mathrm"+"{softclip}(m/"+chr(92)+"hat"+" h)").scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex(chr(92)+"Delta"+"="+chr(92)+"mathrm"+"{softclip}(m/"+chr(92)+"hat"+" h)").scale(0.68)
