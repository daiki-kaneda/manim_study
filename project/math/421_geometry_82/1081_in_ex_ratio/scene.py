...
class InExRatio(PacedScene):
    """#1081 内心傍心比（約45秒）"""

    def construct(self):
        self.show_heading("内心傍心比")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("rとraの比", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("半周長", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.9)
        self.read(0.25)
        cap2 = self.ja_text("面積共通", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq2 = MathTex("r/r_a=(s-a)/s").scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex("r/r_a=(s-a)/s").scale(0.68)
