...
class CircumInRatio(PacedScene):
    """#1082 外接内接比（約45秒）"""

    def construct(self):
        self.show_heading("外接内接比")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("Rとrの比", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("オイラーOI", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.9)
        self.read(0.25)
        cap2 = self.ja_text("形の指標", font_size=24).move_to(self.note)
    ...
    def show_formula(self):
        formula = MathTex("R/r").scale(0.68)
