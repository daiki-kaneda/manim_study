...
class McDiarmid(PacedScene):
    """#1083 マクディアミド（約45秒）"""

    def construct(self):
        self.show_heading("マクディアミド")
        self.draw()
        self.mid()
        self.show_formula()
        self.read(1.4)
    ...
    def draw(self):
        ...
        note = self.ja_text("有界差の集中", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("独立変数", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.9)
        self.read(0.25)
        cap2 = self.ja_text("関数の安定性", font_size=24).move_to(self.note)
