...
class SOAPHard(PacedScene):
    """#1090 SOAPHard（約45秒）"""

    def construct(self):
        self.show_heading("SOAPHard")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("硬前処理SOAP", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("厳密クロネッカー", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.9)
        self.read(0.25)
        cap2 = self.ja_text("高精度", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq2 = MathTex("P=L"+chr(92)+"otimes"+" R").scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex("P=L"+chr(92)+"otimes"+" R").scale(0.68)
