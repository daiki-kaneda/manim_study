...
class InNineDistance(PacedScene):
    """#1116 内心九点距離（約45秒）"""

    def construct(self):
        self.show_heading("内心九点距離")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("IとNの距離", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("九点円中心", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.9)
        self.read(0.25)
        cap2 = self.ja_text("オイラー線近傍", font_size=24).move_to(self.note)
