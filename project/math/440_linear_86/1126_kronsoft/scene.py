...
class KronSoft(PacedScene):
    """#1126 KronSoft（約45秒）"""

    def construct(self):
        self.show_heading("KronSoft")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("Kron軟閾値", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("曲率近似", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.9)
        self.read(0.25)
        cap2 = self.ja_text("平滑更新", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq2 = MathTex(chr(92)+"Delta"+chr(92)+"leftarrow"+chr(92)+"mathrm"+'{soft}(G,A)').scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex(chr(92)+"Delta"+chr(92)+"leftarrow"+chr(92)+"mathrm"+'{soft}(G,A)').scale(0.68)
