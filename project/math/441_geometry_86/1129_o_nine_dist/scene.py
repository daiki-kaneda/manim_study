...
class ONineDistance(PacedScene):
    """#1129 外心九点距離（約45秒）"""

    def construct(self):
        self.show_heading("外心九点距離")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("OとN", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("半径半分", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.9)
        self.read(0.25)
        cap2 = self.ja_text("ON=R/2", font_size=24).move_to(self.note)
    ...
    def show_formula(self):
        formula = MathTex('ON=R/2').scale(0.68)
