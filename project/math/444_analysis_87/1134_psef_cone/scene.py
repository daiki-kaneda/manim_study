...
class PsefCone(PacedScene):
    """#1134 擬有効錐（約45秒）"""

    def construct(self):
        self.show_heading("擬有効錐")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("擬有効な類", font_size=24)
    ...
    def mid(self):
        ...
        self.read(0.25)
        cap2 = self.ja_text("数値的性質", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq2 = MathTex(chr(92)+"overline"+'{'+chr(92)+"mathrm"+'{Eff}}(X)').scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex(chr(92)+"overline"+'{'+chr(92)+"mathrm"+'{Eff}}(X)').scale(0.68)
