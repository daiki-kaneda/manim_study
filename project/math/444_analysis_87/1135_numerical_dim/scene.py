...
class NumericalDimension(PacedScene):
    """#1135 数値的次元（約45秒）"""

    def construct(self):
        self.show_heading("数値的次元")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("ネフの成長", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("体積の指数", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.9)
        self.read(0.25)
        cap2 = self.ja_text("κ_σ", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq2 = MathTex(chr(92)+"kappa"+'_'+chr(92)+"sigma"+'(D)').scale(0.62)
        eq2.move_to(eq)
