...
class ShampooHard(PacedScene):
    """#1139 ShampooHard（約45秒）"""

    def construct(self):
        self.show_heading("ShampooHard")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("前処理強化", font_size=24)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq2 = MathTex('G'+chr(92)+"leftarrow"+' L^{-1/2}GR^{-1/2}').scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex('G'+chr(92)+"leftarrow"+' L^{-1/2}GR^{-1/2}').scale(0.68)
