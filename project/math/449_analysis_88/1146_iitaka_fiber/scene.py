...
class IitakaFiber(PacedScene):
    """#1146 飯高ファイバー（約45秒）"""

    def construct(self):
        self.show_heading("飯高ファイバー")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("飯高ファイバー", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("次元κ", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.9)
        self.read(0.25)
        cap2 = self.ja_text("標準写像", font_size=24).move_to(self.note)
