...
class KodairaFiber(PacedScene):
    """#1147 小平ファイバー（約45秒）"""

    def construct(self):
        self.show_heading("小平ファイバー")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("小平次元の繊維", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("像の次元", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.9)
        self.read(0.25)
        cap2 = self.ja_text("一般型へ", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq2 = MathTex(chr(92)+"kappa"+'(X)='+chr(92)+"dim"+chr(92)+"phi"+'(X)').scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex(chr(92)+"kappa"+'(X)='+chr(92)+"dim"+chr(92)+"phi"+'(X)').scale(0.68)
