...
class CanonicalFiber(PacedScene):
    """#1148 標準ファイバー（約45秒）"""

    def construct(self):
        self.show_heading("標準ファイバー")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("標準モデル繊維", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("K相対ネフ", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.9)
        self.read(0.25)
        cap2 = self.ja_text("ファイバー幾何", font_size=24).move_to(self.note)
    ...
    def show_formula(self):
        formula = MathTex('K_{X/Y}').scale(0.68)
