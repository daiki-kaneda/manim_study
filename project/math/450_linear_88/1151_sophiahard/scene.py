...
class SophiaHard(PacedScene):
    """#1151 SophiaHard（約45秒）"""

    def construct(self):
        self.show_heading("SophiaHard")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("二階硬閾値", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("Hessian剪定", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq2 = MathTex(chr(92)+"hat"+' H'+chr(92)+"leftarrow"+chr(92)+"mathrm"+'{hard}(H)').scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex(chr(92)+"hat"+' H'+chr(92)+"leftarrow"+chr(92)+"mathrm"+'{hard}(H)').scale(0.68)
