...
class MovingLemma(PacedScene):
    """#1158 移動補題（約45秒）"""

    def construct(self):
        self.show_heading("移動補題")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("サイクルを動かす", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("交差を整える", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.9)
        self.read(0.25)
        cap2 = self.ja_text("一般位置", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq2 = MathTex(chr(92)+"alpha"+chr(92)+"cdot"+chr(92)+"beta").scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex(chr(92)+"alpha"+chr(92)+"cdot"+chr(92)+"beta").scale(0.68)
