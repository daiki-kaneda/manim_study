...
class KronHard(PacedScene):
    """#1162 KronHard（約45秒）"""

    def construct(self):
        self.show_heading("KronHard")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("Kron硬閾値", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("曲率剪定", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.9)
        self.read(0.25)
        cap2 = self.ja_text("スパース", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq = MathTex('G'+chr(92)+"leftarrow"+chr(92)+"mathrm"+'{hard}(G)').scale(0.62)
        ...
        self.read(0.2)
        eq2 = MathTex('G'+chr(92)+"leftarrow"+chr(92)+"mathrm"+'{hard}(G)').scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex('G'+chr(92)+"leftarrow"+chr(92)+"mathrm"+'{hard}(G)').scale(0.68)
