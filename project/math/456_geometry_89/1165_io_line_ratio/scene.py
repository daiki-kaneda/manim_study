...
class IOLineRatio(PacedScene):
    """#1165 内心外心線比（約45秒）"""

    def construct(self):
        self.show_heading("内心外心線比")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("OI線分比", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("角の関係", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.9)
        self.read(0.25)
        cap2 = self.ja_text("オイラー距離", font_size=24).move_to(self.note)
    ...
    def show_formula(self):
        formula = MathTex('OI/R').scale(0.68)
