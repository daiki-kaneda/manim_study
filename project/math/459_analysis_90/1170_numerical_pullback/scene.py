...
class NumericalPullback(PacedScene):
    """#1170 数値的引き算（約45秒）"""

    def construct(self):
        self.show_heading("数値的引き算")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("数値的引き戻し", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("射の間", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.9)
        self.read(0.25)
        cap2 = self.ja_text("ネフ性保存", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq2 = MathTex('f^*'+chr(92)+"alpha").scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex('f^*'+chr(92)+"alpha").scale(0.68)
