...
class NefThreshold(PacedScene):
    """#1183 ネフ閾値（約45秒）"""

    def construct(self):
        self.show_heading("ネフ閾値")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("ネフになる境", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("数値的境界", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.9)
        self.read(0.25)
        cap2 = self.ja_text("τ(D)", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq2 = MathTex(chr(92)+"tau"+'(D)').scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex(chr(92)+"tau"+'(D)').scale(0.68)
