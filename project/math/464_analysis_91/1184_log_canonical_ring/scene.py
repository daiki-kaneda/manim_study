...
class LogCanonicalRing(PacedScene):
    """#1184 対数標準環（約45秒）"""

    def construct(self):
        self.show_heading("対数標準環")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("対数標準環", font_size=24)
    ...
    def mid(self):
        ...
        self.read(0.25)
        cap2 = self.ja_text("Proj像", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq = MathTex('R(X,K_X+D)').scale(0.62)
        ...
        self.read(0.2)
        eq2 = MathTex('R(X,K_X+D)').scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex('R(X,K_X+D)').scale(0.68)
