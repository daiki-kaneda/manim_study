...
class AmpleThreshold(PacedScene):
    """#1196 豊富閾値（約45秒）"""

    def construct(self):
        self.show_heading("豊富閾値")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("豊富になる境", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("ネフの内部", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.9)
        self.read(0.25)
        cap2 = self.ja_text("数値判定", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq2 = MathTex(chr(92)+"epsilon"+'(D)').scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex(chr(92)+"epsilon"+'(D)').scale(0.68)
