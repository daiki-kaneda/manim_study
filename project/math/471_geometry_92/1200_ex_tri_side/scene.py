...
class ExTriSide(PacedScene):
    """#1200 傍心三角形辺（約45秒）"""

    def construct(self):
        self.show_heading("傍心三角形辺")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("傍心三角形", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("辺長関係", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.9)
        self.read(0.25)
        cap2 = self.ja_text("外接半径", font_size=24).move_to(self.note)
    ...
    def show_formula(self):
        formula = MathTex('I_aI_b').scale(0.68)
