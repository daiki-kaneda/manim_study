...
class LinearEquivDiff(PacedScene):
    """#1208 線形同値差（約45秒）"""

    def construct(self):
        self.show_heading("線形同値差")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("線形同値の差", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("有理関数", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.9)
        self.read(0.25)
        cap2 = self.ja_text("カルティエ", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq2 = MathTex('D'+chr(92)+"sim"+" D'").scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex('D'+chr(92)+"sim"+" D'").scale(0.68)
