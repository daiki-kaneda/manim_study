...
class NagelRatio(PacedScene):
    """#1213 ナーゲル比（約45秒）"""

    def construct(self):
        self.show_heading("ナーゲル比")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("ナーゲル点", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("傍接辺", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.9)
        self.read(0.25)
        cap2 = self.ja_text("チェバ", font_size=24).move_to(self.note)
