...
class MittenpunktRatio(PacedScene):
    """#1224 ミッテンプンクト比（約45秒）"""

    def construct(self):
        self.show_heading("ミッテンプンクト比")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("ミッテンプンクト", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("傍心チェバ", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.9)
        self.read(0.25)
        cap2 = self.ja_text("比表現", font_size=24).move_to(self.note)
    ...
    def show_formula(self):
        formula = MathTex('M/R').scale(0.68)
