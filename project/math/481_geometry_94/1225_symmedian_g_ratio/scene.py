...
class SymmedianGRatio(PacedScene):
    """#1225 類似重心比（約45秒）"""

    def construct(self):
        self.show_heading("類似重心比")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def mid(self):
        cap = self.ja_text("重心との比", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.9)
        self.read(0.25)
        cap2 = self.ja_text("辺長二乗", font_size=24).move_to(self.note)
