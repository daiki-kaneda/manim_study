...
class VolumePolynomial(PacedScene):
    """#1232 体積多項式（約45秒）"""

    def construct(self):
        self.show_heading("体積多項式")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("体積の多項式性", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("ビッグ錐", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.9)
        self.read(0.25)
        cap2 = self.ja_text("連続性", font_size=24).move_to(self.note)
