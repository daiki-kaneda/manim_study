...
class BigConeBoundary(PacedScene):
    """#1242 ビッグ錐境界（約45秒）"""

    def construct(self):
        self.show_heading("ビッグ錐境界")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("ビッグ錐の縁", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("体積零", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.9)
        self.read(0.25)
        cap2 = self.ja_text("擬有効", font_size=24).move_to(self.note)
    ...
    def show_formula(self):
        formula = MathTex("vol=0").scale(0.68)
