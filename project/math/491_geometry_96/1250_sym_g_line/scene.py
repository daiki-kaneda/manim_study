...
class SymGLine(PacedScene):
    """#1250 類似重心線（約45秒）"""

    def construct(self):
        self.show_heading("類似重心線")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("KとGを結ぶ", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("中線関係", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.9)
        self.read(0.25)
        cap2 = self.ja_text("比分割", font_size=24).move_to(self.note)
