...
class FractionalPart(PacedScene):
    """#1256 小数部（約45秒）"""

    def construct(self):
        self.show_heading("小数部")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("小数部因子", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("丸め差", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq2 = MathTex(chr(92)+"{"+'D'+chr(92)+"}"+'=D-'+chr(92)+"lfloor"+' D'+chr(92)+"rfloor").scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex(chr(92)+"{"+'D'+chr(92)+"}"+'=D-'+chr(92)+"lfloor"+' D'+chr(92)+"rfloor").scale(0.68)
