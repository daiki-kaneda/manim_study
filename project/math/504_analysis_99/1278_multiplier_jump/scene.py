...
class MultiplierJump(PacedScene):
    """#1278 乗法イデアル跳躍（約45秒）"""

    def construct(self):
        self.show_heading("乗法イデアル跳躍")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("跳躍数再訪", font_size=24)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq2 = MathTex(chr(92)+"mathcal"+"{J}(cD)").scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex(chr(92)+"mathcal"+"{J}(cD)").scale(0.68)
