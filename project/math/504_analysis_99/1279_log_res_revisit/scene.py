...
class LogResRevisit(PacedScene):
    """#1279 対数解消再訪（約45秒）"""

    def construct(self):
        self.show_heading("対数解消再訪")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("対数解消", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("SNC境界", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.9)
        self.read(0.25)
        cap2 = self.ja_text("引き戻し", font_size=24).move_to(self.note)
    ...
    def show_formula(self):
        formula = MathTex('f^*(K+D)').scale(0.68)
