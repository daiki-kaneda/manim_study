...
class SNCRevisit(PacedScene):
    """#1280 単純正規交叉再訪（約45秒）"""

    def construct(self):
        self.show_heading("単純正規交叉再訪")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("SNC条件", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("局所座標", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.9)
        self.read(0.25)
        cap2 = self.ja_text("横断交差", font_size=24).move_to(self.note)
