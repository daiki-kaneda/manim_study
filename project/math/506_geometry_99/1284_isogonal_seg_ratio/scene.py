...
class IsogonalSegRatio(PacedScene):
    """#1284 等角線分比（約45秒）"""

    def construct(self):
        self.show_heading("等角線分比")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("等角線分", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("角の対称", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.9)
        self.read(0.25)
        cap2 = self.ja_text("長さ比", font_size=24).move_to(self.note)
