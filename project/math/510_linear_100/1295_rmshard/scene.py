...
class RMSHard(PacedScene):
    """#1295 RMSHard（約45秒）"""

    def construct(self):
        self.show_heading("RMSHard")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("RMS硬閾値", font_size=24)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq = MathTex('v'+chr(92)+"leftarrow"+chr(92)+"mathrm"+'{hard}(v)').scale(0.62)
    ...
    def show_formula(self):
        formula = MathTex('v'+chr(92)+"leftarrow"+chr(92)+"mathrm"+'{hard}(v)').scale(0.68)
