...
class VanObelRatio(PacedScene):
    """#1297 ヴァンオーベル比（約45秒）"""

    def construct(self):
        self.show_heading("ヴァンオーベル比")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("ヴァンオーベル", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("チェバ補助", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq2 = MathTex(chr(92)+"frac"+'{BD}{DC}').scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex(chr(92)+"frac"+'{BD}{DC}').scale(0.68)
