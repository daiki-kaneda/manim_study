...
class LookaheadHard(PacedScene):
    """#1306 LookaheadHard（約45秒）"""

    def construct(self):
        self.show_heading("LookaheadHard")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("先読み硬閾", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("同期剪定", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq2 = MathTex(chr(92)+"phi"+chr(92)+"leftarrow"+chr(92)+"mathrm"+'{hard}('+chr(92)+"phi"+')').scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex(chr(92)+"phi"+chr(92)+"leftarrow"+chr(92)+"mathrm"+'{hard}('+chr(92)+"phi"+')').scale(0.68)
