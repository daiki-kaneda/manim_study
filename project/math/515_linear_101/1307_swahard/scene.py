...
class SWAHard(PacedScene):
    """#1307 SWAHard（約45秒）"""

    def construct(self):
        self.show_heading("SWAHard")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def mid(self):
        cap = self.ja_text("軌跡剪定", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq2 = MathTex(chr(92)+"bar"+' w'+chr(92)+"leftarrow"+chr(92)+"mathrm"+'{hard}('+chr(92)+"bar"+' w)').scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex(chr(92)+"bar"+' w'+chr(92)+"leftarrow"+chr(92)+"mathrm"+'{hard}('+chr(92)+"bar"+' w)').scale(0.68)
