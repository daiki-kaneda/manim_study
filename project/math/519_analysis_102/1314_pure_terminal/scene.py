...
class PureTerminalThreshold(PacedScene):
    """#1314 純端末閾値（約45秒）"""

    def construct(self):
        self.show_heading("純端末閾値")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("純端末", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("例外なし", font_size=24).move_to(self.note)
    ...
    def show_formula(self):
        formula = MathTex('a(E)>0').scale(0.68)
