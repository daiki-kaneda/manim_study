...
class EpsLogTerminal(PacedScene):
    """#1316 ε対数端末（約45秒）"""

    def construct(self):
        self.show_heading("ε対数端末")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("イプシロンlt", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("食い違い下限", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.9)
        self.read(0.25)
        cap2 = self.ja_text("変形", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq2 = MathTex('a(E)>-1+'+chr(92)+"epsilon").scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex('a(E)>-1+'+chr(92)+"epsilon").scale(0.68)
