...
class ProdigyHard(PacedScene):
    """#1317 ProdigyHard（約45秒）"""

    def construct(self):
        self.show_heading("ProdigyHard")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("D推定硬閾", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("学習率剪定", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq = MathTex('d'+chr(92)+"leftarrow"+chr(92)+"mathrm"+'{hard}(d)').scale(0.62)
    ...
    def show_formula(self):
        formula = MathTex('d'+chr(92)+"leftarrow"+chr(92)+"mathrm"+'{hard}(d)').scale(0.68)
