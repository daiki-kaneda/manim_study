...
class Chaining(PacedScene):
    """#1323 チェイニング（約45秒）"""

    def construct(self):
        self.show_heading("チェイニング")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def mid(self):
        cap = self.ja_text("網の階層", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq2 = MathTex(chr(92)+"gamma"+'_2').scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex(chr(92)+"gamma"+'_2').scale(0.68)
