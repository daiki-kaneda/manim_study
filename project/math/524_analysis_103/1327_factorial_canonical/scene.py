...
class FactorialCanonical(PacedScene):
    """#1327 因子的正準（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("因子的正準")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("因子的正準", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("非負の食い違い", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.9)
        self.read(0.25)
        cap2 = self.ja_text("境界つき", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq2 = MathTex('a(E)'+chr(92)+"ge"+' 0').scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex('a(E)'+chr(92)+"ge"+' 0').scale(0.68)
