...
class FactorialKLT(PacedScene):
    """#1328 因子的klt（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("因子的klt")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("因子的klt", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("強い端末", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.9)
        self.read(0.25)
        cap2 = self.ja_text("対数端末", font_size=24).move_to(self.note)
