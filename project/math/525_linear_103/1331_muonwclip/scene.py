...
class MuonWClip(PacedScene):
    """#1331 MuonWClip（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("MuonWClip")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("Muonに重み減衰", font_size=24)
    ...
    def derive(self):
        cap = self.ja_text("Muon核", font_size=24).move_to(self.note)
        ...
        self.read(0.2)
        eq2 = MathTex('U'+chr(92)+"leftarrow"+' U-'+chr(92)+"lambda"+' W').scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex('W'+chr(92)+"leftarrow"+chr(92)+"mathrm"+'{clip}(W-'+chr(92)+"eta"+' U-'+chr(92)+"lambda"+' W)').scale(0.68)
