...
class StewartMedian(PacedScene):
    """#1332 スチュワート中線（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("スチュワート中線")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("スチュワート", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("中線の長さ", font_size=24).move_to(self.note)
    ...
    def derive(self):
        cap = self.ja_text("公式の核", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.75)
        self.read(0.2)
        eq = MathTex('a(d^2+mn)=b^2m+c^2n').scale(0.62)
        eq.to_edge(DOWN, buff=0.2)
        self.play(Write(eq), run_time=1.35)
        self.read(0.3)
        cap2 = self.ja_text("中線特例", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap2), run_time=0.75)
        self.read(0.2)
        eq2 = MathTex('m_a='+chr(92)+"frac"+'{1}{2}'+chr(92)+"sqrt"+'{2b^2+2c^2-a^2}').scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex('m_a='+chr(92)+"frac"+'{1}{2}'+chr(92)+"sqrt"+'{2b^2+2c^2-a^2}').scale(0.68)
