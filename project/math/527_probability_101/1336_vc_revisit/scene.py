...
class VCRevisit(PacedScene):
    """#1336 VC次元再訪（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("VC次元再訪")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def mid(self):
        cap = self.ja_text("粉砕", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.9)
        self.read(0.25)
        cap2 = self.ja_text("成長関数", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq = MathTex('VCdim(H)=d').scale(0.62)
        eq.to_edge(DOWN, buff=0.2)
        self.play(Write(eq), run_time=1.35)
        self.read(0.3)
        cap2 = self.ja_text("サウアーへ", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap2), run_time=0.75)
        self.read(0.2)
        eq2 = MathTex(chr(92)+"Pi"+'_H(n)'+chr(92)+"le"+chr(92)+"sum"+'_{i=0}^d'+chr(92)+"binom"+'{n}{i}').scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex('VCdim=d').scale(0.68)
