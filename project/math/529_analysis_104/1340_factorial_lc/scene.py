...
class FactorialLC(PacedScene):
    """#1340 因子的lc（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("因子的lc")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("因子的lc", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("対数正準", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.9)
        self.read(0.25)
        cap2 = self.ja_text("下限-1", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.3)
        cap2 = self.ja_text("lc", font_size=24).move_to(self.note)
