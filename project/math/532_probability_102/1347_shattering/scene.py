...
class ShatteringNumber(PacedScene):
    """#1347 ショック数（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("ショック数")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("ショック数", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("粉砕能力", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.9)
        self.read(0.25)
        cap2 = self.ja_text("二値ラベル", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq2 = MathTex('S_H(n)'+chr(92)+"le"+chr(92)+"Pi"+'_H(n)').scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex('S_H(n)'+chr(92)+"le"+chr(92)+"Pi"+'_H(n)').scale(0.68)
