...
class GrowthFunction(PacedScene):
    """#1348 成長関数（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("成長関数")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("成長関数", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("標本上の多様度", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.9)
        self.read(0.25)
        cap2 = self.ja_text("VCと結ぶ", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq = MathTex(chr(92)+"Pi"+'_H(n)').scale(0.62)
        eq.to_edge(DOWN, buff=0.2)
        self.play(Write(eq), run_time=1.35)
        self.read(0.3)
        cap2 = self.ja_text("多項式界", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap2), run_time=0.75)
        self.read(0.2)
        eq2 = MathTex(chr(92)+"Pi"+'_H(n)'+chr(92)+"le"+' n^d+1').scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex(chr(92)+"Pi"+'_H(n)'+chr(92)+"le"+' n^d+1').scale(0.68)
