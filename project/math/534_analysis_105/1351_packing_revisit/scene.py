...
class PackingRevisit(PacedScene):
    """#1351 パック数再訪（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("パック数再訪")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("パック数", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("分離半径", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.9)
        self.read(0.25)
        cap2 = self.ja_text("被覆と双対", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq2 = MathTex('M('+chr(92)+"epsilon"+')'+chr(92)+"le"+' N('+chr(92)+"epsilon"+'/2)').scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex('M('+chr(92)+"epsilon"+')'+chr(92)+"le"+' N('+chr(92)+"epsilon"+'/2)').scale(0.68)
