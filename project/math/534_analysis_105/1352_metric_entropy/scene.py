...
class MetricEntropy(PacedScene):
    """#1352 計量エントロピー（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("計量エントロピー")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("計量エントロピー", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("被覆の対数", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq = MathTex('H('+chr(92)+"epsilon"+')='+chr(92)+"log"+' N('+chr(92)+"epsilon"+')').scale(0.62)
        eq.to_edge(DOWN, buff=0.2)
        self.play(Write(eq), run_time=1.35)
        self.read(0.3)
        cap2 = self.ja_text("ダドリーへ", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap2), run_time=0.75)
        self.read(0.2)
        eq2 = MathTex(chr(92)+"int"+chr(92)+"sqrt"+'{H('+chr(92)+"epsilon"+')}'+chr(92)+","+'d'+chr(92)+"epsilon").scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex('H('+chr(92)+"epsilon"+')='+chr(92)+"log"+' N('+chr(92)+"epsilon"+')').scale(0.68)
