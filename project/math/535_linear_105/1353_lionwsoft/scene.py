...
class LionWSoft(PacedScene):
    """#1353 LionWSoft（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("LionWSoft")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("LionW軟化", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("符号+減衰", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq2 = MathTex('x'+chr(92)+"leftarrow"+chr(92)+"mathrm"+'{soft}(x-'+chr(92)+"lambda"+' x)').scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex('x'+chr(92)+"leftarrow"+chr(92)+"mathrm"+'{soft}(x-'+chr(92)+"eta"+chr(92)+"mathrm"+'{sign}(m)-'+chr(92)+"lambda"+' x)').scale(0.68)
