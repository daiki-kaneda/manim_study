...
class MuonWSoft(PacedScene):
    """#1355 MuonWSoft（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("MuonWSoft")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("MuonW軟化", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("直交+減衰", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq = MathTex('U'+chr(92)+"leftarrow"+chr(92)+"mathrm"+'{Muon}(G)').scale(0.62)
    ...
    def show_formula(self):
        formula = MathTex('W'+chr(92)+"leftarrow"+chr(92)+"mathrm"+'{soft}(W-'+chr(92)+"eta"+' U-'+chr(92)+"lambda"+' W)').scale(0.68)
