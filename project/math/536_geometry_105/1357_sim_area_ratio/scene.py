...
class SimAreaRatio(PacedScene):
    """#1357 相似比面積（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("相似比面積")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("相似比", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("線型比k", font_size=24).move_to(self.note)
    ...
    def derive(self):
        cap = self.ja_text("線の比", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.75)
        self.read(0.2)
        eq = MathTex(chr(92)+"ell"+"'/"+chr(92)+"ell"+'=k').scale(0.62)
        ...
        self.read(0.2)
        eq2 = MathTex("S'/S=k^2").scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex("S'/S=k^2").scale(0.68)
