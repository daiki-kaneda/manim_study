...
class MixedDivisor(PacedScene):
    """#1364 混合因子（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("混合因子")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("境界つき混合", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("K+D", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.9)
        self.read(0.25)
        cap2 = self.ja_text("対数ペア", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap2), run_time=0.8)
        self.read(0.35)

    def derive(self):
        cap = self.ja_text("標準+境界", font_size=24).move_to(self.note)
    ...
    def show_formula(self):
        formula = MathTex('K_X+D').scale(0.68)
