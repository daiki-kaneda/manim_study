...
class NineChordRatio(PacedScene):
    """#1368 九点円弦比（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("九点円弦比")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("九点円の弦", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("中点弦", font_size=24).move_to(self.note)
    ...
    def derive(self):
        cap = self.ja_text("弦長", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.75)
        self.read(0.2)
        eq = MathTex('c_N').scale(0.62)
        eq.to_edge(DOWN, buff=0.2)
        self.play(Write(eq), run_time=1.35)
        self.read(0.3)
        cap2 = self.ja_text("半径比", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap2), run_time=0.75)
        self.read(0.2)
        eq2 = MathTex('c_N/R').scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex('c_N/R').scale(0.68)
