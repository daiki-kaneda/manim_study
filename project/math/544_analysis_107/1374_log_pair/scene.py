...
class LogPair(PacedScene):
    """#1374 対数ペア（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("対数ペア")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("対数ペア", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("境界条件", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.9)
        self.read(0.25)
        cap2 = self.ja_text("端末性", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap2), run_time=0.8)
        self.read(0.35)

    def derive(self):
        cap = self.ja_text("ペア", font_size=24).move_to(self.note)
        ...
        self.read(0.2)
        eq2 = MathTex('a(E;X,D)').scale(0.62)
        eq2.move_to(eq)
