...
class RelLogPair(PacedScene):
    """#1375 相対対数ペア（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("相対対数ペア")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("相対対数", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("ファイバー上", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.9)
        self.read(0.25)
        cap2 = self.ja_text("K+D", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq = MathTex('K_{X/S}+D').scale(0.62)
    ...
    def show_formula(self):
        formula = MathTex('(X/S,D)').scale(0.68)
