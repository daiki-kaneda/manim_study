...
class AdaGradSoft(PacedScene):
    """#1377 AdaGradSoft（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("AdaGradSoft")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("累積勾配軟化", font_size=24)
    ...
    def derive(self):
        cap = self.ja_text("AdaGrad", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.75)
        self.read(0.2)
        eq = MathTex('G'+chr(92)+"leftarrow"+' G+g^2').scale(0.62)
        ...
        self.read(0.2)
        eq2 = MathTex('G'+chr(92)+"leftarrow"+chr(92)+"mathrm"+'{soft}(G)').scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex(chr(92)+"eta"+'/'+chr(92)+"sqrt"+'{G+'+chr(92)+"epsilon"+'}').scale(0.68)
