...
class NAdamSoft(PacedScene):
    """#1378 NAdamSoft（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("NAdamSoft")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("Nesterov軟化", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("加速平滑", font_size=24).move_to(self.note)
    ...
    def derive(self):
        cap = self.ja_text("NAdam核", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.75)
        self.read(0.2)
        eq = MathTex('m'+chr(92)+"leftarrow"+chr(92)+"beta"+' m+g').scale(0.62)
        ...
        self.read(0.2)
        eq2 = MathTex('m'+chr(92)+"leftarrow"+chr(92)+"mathrm"+'{soft}(m)').scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex('x'+chr(92)+"leftarrow"+' x-'+chr(92)+"eta"+chr(92)+"mathrm"+'{soft}(m)').scale(0.68)
