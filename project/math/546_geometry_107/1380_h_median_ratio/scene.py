...
class HMedianRatio(PacedScene):
    """#1380 垂心中線比（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("垂心中線比")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("垂心と中線", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("交点比", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.9)
        self.read(0.25)
        cap2 = self.ja_text("オイラー", font_size=24).move_to(self.note)
    ...
    def show_formula(self):
        formula = MathTex('HM/m_a').scale(0.68)
