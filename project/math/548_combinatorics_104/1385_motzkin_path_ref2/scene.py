...
class MotzkinPathRef2(PacedScene):
    """#1385 モツキン細分路（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("モツキン細分路")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def mid(self):
        cap = self.ja_text("水準細分", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.9)
        self.read(0.25)
        cap2 = self.ja_text("計数", font_size=24).move_to(self.note)
    ...
    def show_formula(self):
        formula = MathTex('M_{n,k}').scale(0.68)
