...
class PLTPair(PacedScene):
    """#1387 純対数ペア（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("純対数ペア")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("純対数ペア", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("plt条件", font_size=24).move_to(self.note)
    ...
    def derive(self):
        cap = self.ja_text("条件", font_size=24).move_to(self.note)
    ...
    def show_formula(self):
        formula = MathTex('(X,D)').scale(0.68)
