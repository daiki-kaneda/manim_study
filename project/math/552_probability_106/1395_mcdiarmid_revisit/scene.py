...
class McDiarmidRevisit(PacedScene):
    """#1395 マクディアミド再訪（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("マクディアミド再訪")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("有界差再訪", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("関数安定性", font_size=24).move_to(self.note)
    ...
    def derive(self):
        cap = self.ja_text("差条件", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.75)
        self.read(0.2)
        eq = MathTex('|f(x)-f(x_i)|'+chr(92)+"le"+' c_i').scale(0.62)
