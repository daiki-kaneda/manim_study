...
class SchroederRefine(PacedScene):
    """#1397 シュレーダー細分（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("シュレーダー細分")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("シュレーダー細分", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("括弧列", font_size=24).move_to(self.note)
    ...
    def show_formula(self):
        formula = MathTex('S_{n,k}').scale(0.68)
