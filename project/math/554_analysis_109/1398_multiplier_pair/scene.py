...
class MultiplierPair(PacedScene):
    """#1398 乗法イデアル対（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("乗法イデアル対")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("乗法イデアル", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("対との関係", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.9)
        self.read(0.25)
        cap2 = self.ja_text("消滅へ", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq = MathTex(chr(92)+"mathcal"+'{J}(X,D)').scale(0.62)
        eq.to_edge(DOWN, buff=0.2)
        self.play(Write(eq), run_time=1.35)
        self.read(0.3)
        cap2 = self.ja_text("対へ", font_size=24).move_to(self.note)
    ...
    def show_formula(self):
        formula = MathTex(chr(92)+"mathcal"+'{J}(X,D)').scale(0.68)
