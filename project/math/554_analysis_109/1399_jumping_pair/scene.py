...
class JumpingPair(PacedScene):
    """#1399 跳躍イデアル対（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("跳躍イデアル対")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("跳躍数", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("イデアル変化", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.9)
        self.read(0.25)
        cap2 = self.ja_text("閾値列", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap2), run_time=0.8)
        self.read(0.35)

    def derive(self):
        cap = self.ja_text("跳躍", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.75)
        self.read(0.2)
        eq = MathTex('c_i').scale(0.62)
        ...
        self.read(0.2)
        eq2 = MathTex(chr(92)+"mathcal"+'{J}(X,cD)').scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex('c_i').scale(0.68)
