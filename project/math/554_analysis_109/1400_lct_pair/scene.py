...
class LCTPair(PacedScene):
    """#1400 対数的閾値対（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("対数的閾値対")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("LCT", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("対数ペア", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.9)
        self.read(0.25)
        cap2 = self.ja_text("最小跳躍", font_size=24).move_to(self.note)
    ...
    def show_formula(self):
        formula = MathTex("lct(X,D)").scale(0.68)
