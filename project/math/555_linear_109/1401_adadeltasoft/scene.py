...
class AdaDeltaSoft(PacedScene):
    """#1401 AdaDeltaSoft（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("AdaDeltaSoft")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("RMS比軟化", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("更新幅平滑", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.9)
        self.read(0.25)
        cap2 = self.ja_text("適応刻み", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq = MathTex(chr(92)+"Delta"+' x'+chr(92)+"leftarrow"+chr(92)+"mathrm"+'{RMS}').scale(0.62)
        ...
        self.read(0.2)
        eq2 = MathTex(chr(92)+"Delta"+' x'+chr(92)+"leftarrow"+chr(92)+"mathrm"+'{soft}('+chr(92)+"Delta"+' x)').scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex(chr(92)+"Delta"+' x'+chr(92)+"leftarrow"+chr(92)+"mathrm"+'{soft}('+chr(92)+"mathrm"+'{RMS})').scale(0.68)
