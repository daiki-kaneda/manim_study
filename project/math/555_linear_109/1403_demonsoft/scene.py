...
class DemonSoft(PacedScene):
    """#1403 DemonSoft（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("DemonSoft")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("減衰係数軟化", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("モーメンタム", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq = MathTex(chr(92)+"beta"+'_t').scale(0.62)
        ...
        self.read(0.2)
        eq2 = MathTex(chr(92)+"beta"+'_t'+chr(92)+"leftarrow"+chr(92)+"mathrm"+'{soft}('+chr(92)+"beta"+'_t)').scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex('v'+chr(92)+"leftarrow"+chr(92)+"mathrm"+'{soft}('+chr(92)+"beta"+'_t)v+g').scale(0.68)
