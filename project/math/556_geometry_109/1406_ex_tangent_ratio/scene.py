...
class ExTangentRatio(PacedScene):
    """#1406 傍接弦比（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("傍接弦比")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("傍接線", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("弦の比", font_size=24).move_to(self.note)
    ...
    def derive(self):
        cap = self.ja_text("接線長", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.75)
        self.read(0.2)
        eq = MathTex('s-a').scale(0.62)
        ...
        self.read(0.2)
        eq2 = MathTex('t_a/t_b').scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex('t_a=s-a').scale(0.68)
