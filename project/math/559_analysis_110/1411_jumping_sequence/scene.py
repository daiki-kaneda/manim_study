...
class JumpingSequence(PacedScene):
    """#1411 跳躍数列（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("跳躍数列")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("跳躍の列", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("有限性", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.9)
        self.read(0.25)
        cap2 = self.ja_text("閾値", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap2), run_time=0.8)
        self.read(0.35)

    def derive(self):
        cap = self.ja_text("列", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.75)
        self.read(0.2)
        eq = MathTex('c_1<c_2<'+chr(92)+"cdots").scale(0.62)
        eq.to_edge(DOWN, buff=0.2)
        self.play(Write(eq), run_time=1.35)
        self.read(0.3)
        cap2 = self.ja_text("有限", font_size=24).move_to(self.note)
    ...
    def show_formula(self):
        formula = MathTex('c_1<c_2<'+chr(92)+"cdots"+'<c_m').scale(0.68)
