...
class PedalChordRatio(PacedScene):
    """#1417 垂足弦比（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("垂足弦比")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("垂足弦", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("垂足三角形", font_size=24).move_to(self.note)
    ...
    def derive(self):
        cap = self.ja_text("垂足", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.75)
        self.read(0.2)
        eq = MathTex('H_aH_b').scale(0.62)
        ...
        self.read(0.2)
        eq2 = MathTex('H_aH_b/a').scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex('H_aH_b/a').scale(0.68)
