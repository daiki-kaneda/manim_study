...
class NadelPair(PacedScene):
    """#1422 Nadelイデアル対（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("Nadelイデアル対")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("Nadel", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("乗法イデアル", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.9)
        self.read(0.25)
        cap2 = self.ja_text("消滅", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq = MathTex(chr(92)+"mathcal"+'{J}(D)').scale(0.62)
        ...
        self.read(0.2)
        eq2 = MathTex('H^i(K+L'+chr(92)+"otimes"+chr(92)+"mathcal"+'{J})=0').scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex('H^i=0').scale(0.68)
