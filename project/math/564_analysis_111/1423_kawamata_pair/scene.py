...
class KawamataPair(PacedScene):
    """#1423 川又対（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("川又対")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("川又・フィーベック", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("対数対", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq2 = MathTex('H^i(X,K+D+L)=0').scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex('H^i(K+D+L)=0').scale(0.68)
