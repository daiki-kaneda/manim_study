...
class TerminalVolume(PacedScene):
    """#1424 端末対の体積（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("端末対の体積")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("端末対", font_size=24)
    ...
    def mid(self):
        ...
        self.read(0.25)
        cap2 = self.ja_text("正値", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap2), run_time=0.8)
        self.read(0.35)

    def derive(self):
        cap = self.ja_text("端末", font_size=24).move_to(self.note)
    ...
    def show_formula(self):
        formula = MathTex('vol(K_X+D)').scale(0.68)
