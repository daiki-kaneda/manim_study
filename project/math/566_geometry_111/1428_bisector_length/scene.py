...
class BisectorLength(PacedScene):
    """#1428 角二等分長（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("角二等分長")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("二等分線長", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("辺長公式", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq = MathTex('t_a^2=bc(1-(a/(b+c))^2)').scale(0.62)
    ...
    def show_formula(self):
        formula = MathTex('t_a^2=bc(1-(a/(b+c))^2)').scale(0.68)
