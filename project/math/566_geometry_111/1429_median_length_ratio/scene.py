...
class MedianLengthRatio(PacedScene):
    """#1429 中線長比（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("中線長比")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("中線長", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("辺の二乗", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq = MathTex('m_a='+chr(92)+"frac"+'{1}{2}'+chr(92)+"sqrt"+'{2b^2+2c^2-a^2}').scale(0.62)
    ...
    def show_formula(self):
        formula = MathTex('m_a/a').scale(0.68)
