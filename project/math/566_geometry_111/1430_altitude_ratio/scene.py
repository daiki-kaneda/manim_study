...
class AltitudeRatio(PacedScene):
    """#1430 高さ比（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("高さ比")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("高さ", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("面積経由", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq = MathTex('h_a=2S/a').scale(0.62)
        ...
        self.read(0.2)
        eq2 = MathTex('h_a/h_b').scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex('h_a/h_b=b/a').scale(0.68)
