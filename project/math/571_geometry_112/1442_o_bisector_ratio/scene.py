...
class OBisectorRatio(PacedScene):
    """#1442 外心角二等分比（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("外心角二等分比")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("外心と二等分", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("円周角", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq2 = MathTex(chr(92)+"ell"+'/'+chr(92)+"ell"+"'").scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex(chr(92)+"ell"+'/'+chr(92)+"ell"+"'").scale(0.68)
