...
class ShatteringCoef(PacedScene):
    """#1444 粉砕係数（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("粉砕係数")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("粉砕係数", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("ラベル多様度", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq2 = MathTex('S(n)'+chr(92)+"le"+chr(92)+"Pi"+'(n)').scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex('S(n)'+chr(92)+"le"+chr(92)+"Pi"+'(n)').scale(0.68)
