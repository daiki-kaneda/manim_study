...
class FussRefine(PacedScene):
    """#1457 フォア細分（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("フォア細分")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        vals = VGroup(*[
            ...
            for i, v in enumerate([1, 3, 12, 55, 273])
        ])
        self.play(LaggedStart(*[FadeIn(v) for v in vals], lag_ratio=0.1), run_time=1.4)
        note = self.ja_text("Fuss–Catalan", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("多角形細分", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.9)
        self.read(0.25)
        cap2 = self.ja_text("配列", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq2 = MathTex('C(n,k;m)').scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex('C(n,k;m)').scale(0.68)
