...
class ExAltitudeRatio(PacedScene):
    """#1465 傍心高さ比（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("傍心高さ比")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("傍心と高さ", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("傍接半径", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq = MathTex('r_a=S/(s-a)').scale(0.62)
        ...
        self.read(0.2)
        eq2 = MathTex('r_a/h_a').scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex('r_a/h_a').scale(0.68)
