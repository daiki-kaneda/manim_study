...
class OAltitudeRatio(PacedScene):
    """#1466 外心高さ比（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("外心高さ比")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("外心と高さ", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("外接半径", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq = MathTex('R=abc/4S').scale(0.62)
    ...
    def show_formula(self):
        formula = MathTex('R/h_a').scale(0.68)
