...
class EpsilonNet(PacedScene):
    """#1467 イプシロンネット（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("イプシロンネット")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("εネット", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("被覆", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq2 = MathTex('|N|'+chr(92)+"lesssim"+chr(92)+"epsilon"+'^{-d}').scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex('|N|'+chr(92)+"lesssim"+chr(92)+"epsilon"+'^{-d}').scale(0.68)
