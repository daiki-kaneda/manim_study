...
class AppelRefine(PacedScene):
    """#1469 アペル細分（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("アペル細分")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def mid(self):
        cap = self.ja_text("微分関係", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq = MathTex("A_n'(x)=nA_{n-1}(x)").scale(0.62)
