...
class NarayanaRefine(PacedScene):
    """#1493 ナーラヤナ細分（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("ナーラヤナ細分")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("ナーラヤナ", font_size=24)
    ...
    def mid(self):
        ...
        self.read(0.25)
        cap2 = self.ja_text("峰数", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq2 = MathTex('C_n='+chr(92)+"sum"+'_k N(n,k)').scale(0.62)
        eq2.move_to(eq)
