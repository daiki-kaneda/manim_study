...
class LocalCovering(PacedScene):
    """#1503 局所被覆数（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("局所被覆数")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("局所被覆", font_size=24)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq = MathTex('N(B(f,r),'+chr(92)+"epsilon"+')').scale(0.62)
        ...
        self.read(0.2)
        eq2 = MathTex('N_{'+chr(92)+"mathrm"+'{loc}}('+chr(92)+"epsilon"+')').scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex('N_{'+chr(92)+"mathrm"+'{loc}}('+chr(92)+"epsilon"+')').scale(0.68)
