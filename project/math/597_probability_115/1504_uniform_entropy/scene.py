...
class UniformEntropy(PacedScene):
    """#1504 一様エントロピー（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("一様エントロピー")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("一様エントロピー", font_size=24)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq = MathTex('H('+chr(92)+"epsilon"+')').scale(0.62)
    ...
    def show_formula(self):
        formula = MathTex(chr(92)+"int"+chr(92)+"sqrt"+'{H('+chr(92)+"epsilon"+')}'+chr(92)+","+'d'+chr(92)+"epsilon").scale(0.68)
