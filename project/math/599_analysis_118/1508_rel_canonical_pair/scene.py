...
class RelCanonicalPair(PacedScene):
    """#1508 相対カノニカル対（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("相対カノニカル対")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("相対カノニカル", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("食い違い非負", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.9)
        self.read(0.25)
        cap2 = self.ja_text("閉条件", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq = MathTex('a(E,X/S,D)').scale(0.62)
        ...
        self.read(0.2)
        eq2 = MathTex('a(E,X/S,D)'+chr(92)+"ge"+' 0').scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex('a(E,X/S,D)'+chr(92)+"ge"+' 0').scale(0.68)
