...
class LucasRefine(PacedScene):
    """#1517 リュカ細分（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("リュカ細分")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("リュカ数", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("漸化式", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq = MathTex('L_n=L_{n-1}+L_{n-2}').scale(0.62)
