...
class ZeckendorfRefine(PacedScene):
    """#1541 ゼッケンドルフ細分（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("ゼッケンドルフ細分")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def mid(self):
        cap = self.ja_text("フィボナッチ和", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq = MathTex('n='+chr(92)+"sum"+' F_{k_i}').scale(0.62)
    ...
    def show_formula(self):
        formula = MathTex('Z(n,k)').scale(0.68)
