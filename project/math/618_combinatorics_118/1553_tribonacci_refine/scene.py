...
class TribonacciRefine(PacedScene):
    """#1553 トリボナッチ細分（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("トリボナッチ細分")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def mid(self):
        cap = self.ja_text("三項漸化", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq = MathTex('T_n=T_{n-1}+T_{n-2}+T_{n-3}').scale(0.62)
    ...
    def show_formula(self):
        formula = MathTex('T(n,k)').scale(0.68)
