...
class LogNefPair(PacedScene):
    """#1554 ログネフ対（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("ログネフ対")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("ログネフ", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("交差非負", font_size=24).move_to(self.note)
