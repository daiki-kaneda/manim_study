...
class LogBigPair(PacedScene):
    """#1555 ログビッグ対（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("ログビッグ対")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("ログビッグ", font_size=24)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq2 = MathTex('vol(L)>0').scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex('vol(L)>0').scale(0.68)
