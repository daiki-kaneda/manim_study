...
class LARSBound(PacedScene):
    """#1558 LARSBound（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("LARSBound")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("LARS境界", font_size=24)
    ...
    def derive(self):
        ...
        self.read(0.3)
        cap2 = self.ja_text("境界", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap2), run_time=0.75)
        self.read(0.2)
        eq2 = MathTex(chr(92)+"eta"+chr(92)+"in"+'[L,U]').scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex(chr(92)+"eta"+chr(92)+"in"+'[L,U]').scale(0.68)
