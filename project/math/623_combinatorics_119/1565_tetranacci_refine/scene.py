...
class TetranacciRefine(PacedScene):
    """#1565 テトラナッチ細分（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("テトラナッチ細分")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def mid(self):
        cap = self.ja_text("四項漸化", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq = MathTex('A_n='+chr(92)+"sum"+'_{i=1}^4 A_{n-i}').scale(0.62)
