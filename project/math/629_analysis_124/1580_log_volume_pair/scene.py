...
class LogVolumePair(PacedScene):
    """#1580 ログ体積対（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("ログ体積対")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("ログ体積", font_size=24)
    ...
    def mid(self):
        ...
        self.read(0.25)
        cap2 = self.ja_text("ビッグ錐", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq2 = MathTex('vol(D)>0').scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex('vol(D)').scale(0.68)
