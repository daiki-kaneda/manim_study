...
class NSteinerRatio(PacedScene):
    """#1586 九点シュタイナー比（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("九点シュタイナー比")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("九点円とシュタイナー", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("楕円", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq2 = MathTex(chr(92)+"ell"+'_S/'+chr(92)+"ell"+'_N').scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex(chr(92)+"ell"+'_S/'+chr(92)+"ell"+'_N').scale(0.68)
