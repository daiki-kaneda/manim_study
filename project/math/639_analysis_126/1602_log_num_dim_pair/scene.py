...
class LogNumDimPair(PacedScene):
    """#1602 ログ数値次元対（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("ログ数値次元対")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("ログ数値次元", font_size=24)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq = MathTex(chr(92)+"kappa"+'_'+chr(92)+"sigma"+'(D)').scale(0.62)
        ...
        self.read(0.2)
        eq2 = MathTex(chr(92)+"kappa"+'_'+chr(92)+"sigma"+'(D)'+chr(92)+"in"+chr(92)+"{"+'0,'+chr(92)+"ldots"+',n'+chr(92)+"}").scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex(chr(92)+"kappa"+'_'+chr(92)+"sigma"+'(D)').scale(0.68)
