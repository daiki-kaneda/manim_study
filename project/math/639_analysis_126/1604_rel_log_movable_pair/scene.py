...
class RelLogMovablePair(PacedScene):
    """#1604 相対ログ移動対（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("相対ログ移動対")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("相対ログ移動", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("正値部", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq2 = MathTex('D'+chr(92)+"in"+' Mov').scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex('D'+chr(92)+"in"+' Mov').scale(0.68)
