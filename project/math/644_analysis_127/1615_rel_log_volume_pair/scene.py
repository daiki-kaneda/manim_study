...
class RelLogVolumePair(PacedScene):
    """#1615 相対ログ体積対（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("相対ログ体積対")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("相対ログ体積", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("連続性", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.9)
        self.read(0.25)
        cap2 = self.ja_text("相対ビッグ", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq = MathTex('vol_{X/S}(D)').scale(0.62)
        ...
        self.read(0.2)
        eq2 = MathTex('vol_{X/S}(D)>0').scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex('vol_{X/S}(D)').scale(0.68)
