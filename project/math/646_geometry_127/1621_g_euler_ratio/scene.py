...
class GEulerRatio(PacedScene):
    """#1621 重心オイラー比（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("重心オイラー比")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("重心とオイラー線", font_size=24)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq2 = MathTex('OG:GH=1:2').scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex('OG:GH=1:2').scale(0.68)
