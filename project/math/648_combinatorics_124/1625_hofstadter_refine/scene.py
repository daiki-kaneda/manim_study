...
class HofstadterRefine(PacedScene):
    """#1625 ホフスタッター細分（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("ホフスタッター細分")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("ホフスタッター", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("入れ子漸化", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq = MathTex('a(n)=n-a(a(n-1))').scale(0.62)
