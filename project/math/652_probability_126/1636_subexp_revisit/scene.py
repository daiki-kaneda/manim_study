...
class SubexpRevisit(PacedScene):
    """#1636 劣指数再訪（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("劣指数再訪")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("劣指数", font_size=24)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq = MathTex(chr(92)+"psi"+'_1').scale(0.62)
        ...
        self.read(0.2)
        eq2 = MathTex(chr(92)+"mathbb"+'{P}(|X|>t)'+chr(92)+"le"+' 2e^{-t/K}').scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex(chr(92)+"mathbb"+'{P}(|X|>t)'+chr(92)+"le"+' 2e^{-t/K}').scale(0.68)
