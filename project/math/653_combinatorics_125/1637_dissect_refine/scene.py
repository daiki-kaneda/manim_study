...
class DissectRefine(PacedScene):
    """#1637 ディセクト細分（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("ディセクト細分")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("多角形細分", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("対角線", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.9)
        self.read(0.25)
        cap2 = self.ja_text("数え上げ", font_size=24).move_to(self.note)
    ...
    def show_formula(self):
        formula = MathTex('D(n,k)').scale(0.68)
