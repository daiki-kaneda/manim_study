...
class RelLogContraction(PacedScene):
    """#1638 相対ログ収縮対（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("相対ログ収縮対")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("相対ログ収縮", font_size=24)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq = MathTex(chr(92)+"phi"+':X'+chr(92)+"to"+' Y').scale(0.62)
        ...
        self.read(0.2)
        eq2 = MathTex(chr(92)+"rho"+'(X/Y)=1').scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex(chr(92)+"rho"+'(X/Y)=1').scale(0.68)
