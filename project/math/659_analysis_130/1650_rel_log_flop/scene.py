...
class RelLogFlop(PacedScene):
    """#1650 相対ログフロップ対（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("相対ログフロップ対")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("相対ログフロップ", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("K自明", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.9)
        self.read(0.25)
        cap2 = self.ja_text("双有理", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq = MathTex('K_X'+chr(92)+"cdot"+' C=0').scale(0.62)
