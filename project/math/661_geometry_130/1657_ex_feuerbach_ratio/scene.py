...
class ExFeuerbachRatio(PacedScene):
    """#1657 傍心フォイエルバッハ比（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("傍心フォイエルバッハ比")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("傍心とフォイエルバッハ", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("九点円接点", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq2 = MathTex('d(I_a,N)/r_a').scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex('d(I_a,N)/r_a').scale(0.68)
