...
class LogTerminalModel(PacedScene):
    """#1662 ログ終端対（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("ログ終端対")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("ログ終端", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("終端モデル", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.9)
        self.read(0.25)
        cap2 = self.ja_text("ネフ", font_size=24).move_to(self.note)
