...
class GFeuerbachRatio(PacedScene):
    """#1669 重心フォイエルバッハ比（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("重心フォイエルバッハ比")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("重心とフォイエルバッハ", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("九点円", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq2 = MathTex('GN/'+chr(92)+"ell").scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex('GN/'+chr(92)+"ell").scale(0.68)
