...
class LogSemistablePair(PacedScene):
    """#1674 ログ半安定対（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("ログ半安定対")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("ログ半安定", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("簡約ファイバー", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.9)
        self.read(0.25)
        cap2 = self.ja_text("平坦", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq2 = MathTex('(X_s,D_s)').scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex('(X_s,D_s)').scale(0.68)
