...
class RelLogStablePair(PacedScene):
    """#1675 相対ログ安定対（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("相対ログ安定対")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("相対ログ安定", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("相対豊富", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.9)
        self.read(0.25)
        cap2 = self.ja_text("平坦族", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq2 = MathTex('(K_X+D)/S').scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex('(K_X+D)/S').scale(0.68)
