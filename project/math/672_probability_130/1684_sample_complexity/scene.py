...
class SampleComplexity(PacedScene):
    """#1684 標本依存複雑度（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("標本依存複雑度")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("標本依存複雑度", font_size=24)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq = MathTex(chr(92)+"mathfrak"+'{C}_n').scale(0.62)
        ...
        self.read(0.2)
        eq2 = MathTex(chr(92)+"mathfrak"+'{C}_n'+chr(92)+"le"+' C/'+chr(92)+"sqrt"+' n').scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex(chr(92)+"mathfrak"+'{C}_n'+chr(92)+"le"+' C/'+chr(92)+"sqrt"+' n').scale(0.68)
