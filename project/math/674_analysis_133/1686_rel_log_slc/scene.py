...
class RelLogSlc(PacedScene):
    """#1686 相対ログ簡約対（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("相対ログ簡約対")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("相対ログ簡約", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("半対数標準", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq2 = MathTex('a(E,X/S,D)'+chr(92)+"ge"+' -1').scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex('a(E,X/S,D)'+chr(92)+"ge"+' -1').scale(0.68)
