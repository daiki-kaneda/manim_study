...
class RelLogFlat(PacedScene):
    """#1688 相対ログ平坦対（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("相対ログ平坦対")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("相対ログ平坦", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("平坦射", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq = MathTex('f:X'+chr(92)+"to"+' S').scale(0.62)
        ...
        self.read(0.2)
        eq2 = MathTex('f:X'+chr(92)+"to"+' S').scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex('f:X'+chr(92)+"to"+' S').scale(0.68)
