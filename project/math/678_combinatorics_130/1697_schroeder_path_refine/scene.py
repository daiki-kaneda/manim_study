...
class SchroederPathRefine(PacedScene):
    """#1697 シュレーダー路細分（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("シュレーダー路細分")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("シュレーダー路", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("弱上昇", font_size=24).move_to(self.note)
