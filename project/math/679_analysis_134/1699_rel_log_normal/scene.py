...
class RelLogNormal(PacedScene):
    """#1699 相対ログ正規対（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("相対ログ正規対")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("相対ログ正規", font_size=24)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq2 = MathTex('D='+chr(92)+"sum"+' D_i').scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex('D='+chr(92)+"sum"+' D_i').scale(0.68)
