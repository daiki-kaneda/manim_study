...
class MotzkinTriRefine(PacedScene):
    """#1709 モツキン三角形細分（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("モツキン三角形細分")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("モツキン三角形", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("配列", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq2 = MathTex('M_n='+chr(92)+"sum"+'_k M(n,k)').scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex('M(n,k)').scale(0.68)
