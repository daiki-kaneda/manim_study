...
class LogSmoothPair(PacedScene):
    """#1711 ログ平滑対（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("ログ平滑対")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("ログ平滑", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("スムース境界", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.9)
        self.read(0.25)
        cap2 = self.ja_text("正規", font_size=24).move_to(self.note)
