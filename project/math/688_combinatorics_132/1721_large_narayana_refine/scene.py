...
class LargeNarayanaRefine(PacedScene):
    """#1721 大きなナラヤナ細分（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("大きなナラヤナ細分")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def mid(self):
        cap = self.ja_text("拡張", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq2 = MathTex('C_n^{(m)}='+chr(92)+"sum"+'_k N(n,k;m)').scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex('N(n,k;m)').scale(0.68)
