...
class PlaneBinTreeRefine(PacedScene):
    """#1733 平面二分木細分（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("平面二分木細分")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("平面二分木", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("左右順序", font_size=24).move_to(self.note)
    ...
    def show_formula(self):
        formula = MathTex('P(n,k)').scale(0.68)
