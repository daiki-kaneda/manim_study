...
class RelLogSnc(PacedScene):
    """#1734 相対ログ純交対（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("相対ログ純交対")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("相対ログ純交", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("相対正規交叉", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq2 = MathTex('D_i'+chr(92)+"pitchfork"+'_S D_j').scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex('D_i'+chr(92)+"pitchfork"+'_S D_j').scale(0.68)
