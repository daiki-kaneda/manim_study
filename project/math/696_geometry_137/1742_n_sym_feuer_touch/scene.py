...
class NSymFeuerTouch(PacedScene):
    """#1742 九点類似フォイエル接点比（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("九点類似フォイエル接点比")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("九点円類似接点", font_size=24)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq2 = MathTex('NT_s/R_N').scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex('NT_s/R_N').scale(0.68)
