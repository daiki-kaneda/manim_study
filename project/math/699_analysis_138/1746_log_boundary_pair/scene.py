...
class LogBoundaryPair(PacedScene):
    """#1746 ログ境界対（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("ログ境界対")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("ログ境界", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("境界因子", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.9)
        self.read(0.25)
        cap2 = self.ja_text("支持", font_size=24).move_to(self.note)
