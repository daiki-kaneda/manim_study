...
class RelLogBoundary(PacedScene):
    """#1747 相対ログ境界対（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("相対ログ境界対")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("相対ログ境界", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("相対境界", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq = MathTex('D='+chr(92)+"sum"+' a_i D_i').scale(0.62)
        ...
        self.read(0.2)
        eq2 = MathTex('0'+chr(92)+"le"+' a_i'+chr(92)+"le"+' 1').scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex('0'+chr(92)+"le"+' a_i'+chr(92)+"le"+' 1').scale(0.68)
