...
class LogPurityPair(PacedScene):
    """#1748 ログ純度対（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("ログ純度対")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("ログ純度", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("純次元", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.9)
        self.read(0.25)
        cap2 = self.ja_text("特異", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq = MathTex(chr(92)+"dim").scale(0.62)
        ...
        self.read(0.2)
        eq2 = MathTex(chr(92)+"dim"+'=d').scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex(chr(92)+"dim"+'=d').scale(0.68)
