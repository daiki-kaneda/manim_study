...
class DataPacking(PacedScene):
    """#1755 データ依存パッキング（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("データ依存パッキング")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("データ依存パッキング", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("標本分離", font_size=24).move_to(self.note)
