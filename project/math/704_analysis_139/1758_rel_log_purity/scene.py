...
class RelLogPurity(PacedScene):
    """#1758 相対ログ純度対（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("相対ログ純度対")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("相対ログ純度", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("相対純次元", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq = MathTex(chr(92)+"dim"+'/S').scale(0.62)
        ...
        self.read(0.2)
        eq2 = MathTex(chr(92)+"dim"+'/S=d').scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex(chr(92)+"dim"+'/S=d').scale(0.68)
