...
class EmpLocalPacking(PacedScene):
    """#1779 経験局所パッキング（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("経験局所パッキング")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("経験局所パッキング", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("データ半径球", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq = MathTex('M_n(B,'+chr(92)+"epsilon"+')').scale(0.62)
        ...
        self.read(0.2)
        eq2 = MathTex('M_{n,'+chr(92)+"mathrm"+'{loc}}('+chr(92)+"epsilon"+')').scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex('M_{n,'+chr(92)+"mathrm"+'{loc}}('+chr(92)+"epsilon"+')').scale(0.68)
