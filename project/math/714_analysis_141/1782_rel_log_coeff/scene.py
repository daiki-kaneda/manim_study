...
class RelLogCoeff(PacedScene):
    """#1782 相対ログ係数対（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("相対ログ係数対")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("相対ログ係数", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("相対境界係数", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.9)
        self.read(0.25)
        cap2 = self.ja_text("区間", font_size=24).move_to(self.note)
