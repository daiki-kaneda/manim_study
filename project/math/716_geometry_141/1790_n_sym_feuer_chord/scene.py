...
class NSymFeuerChord(PacedScene):
    """#1790 九点類似フォイエル弦比（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("九点類似フォイエル弦比")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("九点円類似弦", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("弦", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq2 = MathTex(chr(92)+"ell"+'_{F,s}/R_N').scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex(chr(92)+"ell"+'_{F,s}/R_N').scale(0.68)
