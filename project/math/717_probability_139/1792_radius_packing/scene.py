...
class RadiusPacking(PacedScene):
    """#1792 半径パッキング数（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("半径パッキング数")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("半径パッキング数", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("球分離", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq2 = MathTex('M(r,'+chr(92)+"epsilon"+')'+chr(92)+"lesssim"+' (r/'+chr(92)+"epsilon"+')^d').scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex('M(r,'+chr(92)+"epsilon"+')'+chr(92)+"lesssim"+' (r/'+chr(92)+"epsilon"+')^d').scale(0.68)
