...
class CayleyRefine(PacedScene):
    """#1793 ケイリー細分（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("ケイリー細分")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("ケイリー", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("木の公式", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq2 = MathTex('Cay(n,k)').scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex('Cay(n,k)').scale(0.68)
