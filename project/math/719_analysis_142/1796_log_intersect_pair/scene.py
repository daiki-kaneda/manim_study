...
class LogIntersectPair(PacedScene):
    """#1796 ログ交点数対（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("ログ交点数対")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("ログ交点数", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("交差形式", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq = MathTex('(D_1'+chr(92)+"cdot"+' D_2)').scale(0.62)
        ...
        self.read(0.2)
        eq2 = MathTex('(D_1'+chr(92)+"cdot"+' D_2)'+chr(92)+"in"+chr(92)+"mathbb"+'{Z}').scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex('(D_1'+chr(92)+"cdot"+' D_2)').scale(0.68)
