...
class LocalRadiusPacking(PacedScene):
    """#1804 局所半径パッキング（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("局所半径パッキング")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("局所半径パッキング", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("球内分離", font_size=24).move_to(self.note)
    ...
    def show_formula(self):
        formula = MathTex('M'+chr(92)+"lesssim"+' (r/'+chr(92)+"epsilon"+')^d').scale(0.68)
