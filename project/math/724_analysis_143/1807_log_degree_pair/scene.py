...
class LogDegreePair(PacedScene):
    """#1807 ログ次数対（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("ログ次数対")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("ログ次数", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("因子次数", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq = MathTex(chr(92)+"deg"+'(D)').scale(0.62)
        ...
        self.read(0.2)
        eq2 = MathTex(chr(92)+"deg"+'(D)='+chr(92)+"int"+' c_1(D)').scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex(chr(92)+"deg"+'(D)').scale(0.68)
