...
class LogVolPolyPair(PacedScene):
    """#1818 ログ体積多項式対（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("ログ体積多項式対")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("ログ体積多項式", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("体積展開", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq2 = MathTex('vol(tD)='+chr(92)+"sum"+' a_k t^k').scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex('vol(tD)='+chr(92)+"sum"+' a_k t^k').scale(0.68)
