...
class RelLogVolPoly(PacedScene):
    """#1819 相対ログ体積多項式対（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("相対ログ体積多項式対")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("相対ログ体積多項式", font_size=24)
    ...
    def mid(self):
        ...
        self.read(0.25)
        cap2 = self.ja_text("係数", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq = MathTex('vol_{X/S}(tD)').scale(0.62)
        eq.to_edge(DOWN, buff=0.2)
        self.play(Write(eq), run_time=1.35)
        self.read(0.3)
        cap2 = self.ja_text("展開", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap2), run_time=0.75)
        self.read(0.2)
        eq2 = MathTex('vol='+chr(92)+"sum"+' a_k t^k').scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex('vol='+chr(92)+"sum"+' a_k t^k').scale(0.68)
