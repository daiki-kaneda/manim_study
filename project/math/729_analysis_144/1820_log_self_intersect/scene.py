...
class LogSelfIntersect(PacedScene):
    """#1820 ログ自己交対（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("ログ自己交対")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("ログ自己交", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("自己交差", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq2 = MathTex('D^2=(D'+chr(92)+"cdot"+' D)').scale(0.62)
        eq2.move_to(eq)
