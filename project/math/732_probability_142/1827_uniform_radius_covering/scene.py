...
class UniformRadiusCovering(PacedScene):
    """#1827 一様半径被覆（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("一様半径被覆")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("一様半径被覆", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("全空間球", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq2 = MathTex('N_u'+chr(92)+"lesssim"+' (r/'+chr(92)+"epsilon"+')^d').scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex('N_u'+chr(92)+"lesssim"+' (r/'+chr(92)+"epsilon"+')^d').scale(0.68)
