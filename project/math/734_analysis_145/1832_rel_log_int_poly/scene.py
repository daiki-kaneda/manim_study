...
class RelLogIntPoly(PacedScene):
    """#1832 相対ログ交多項式対（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("相対ログ交多項式対")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("相対ログ交多項式", font_size=24)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq2 = MathTex('(tD)^n='+chr(92)+"sum"+' a_k t^k').scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex('(tD)^n='+chr(92)+"sum"+' a_k t^k').scale(0.68)
