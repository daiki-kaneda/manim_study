...
class NSymFeuerArc(PacedScene):
    """#1838 九点類似フォイエル弧比（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("九点類似フォイエル弧比")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("九点円類似弧", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("弧", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq2 = MathTex(chr(92)+"ell"+'_{a,s}/R_N').scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex(chr(92)+"ell"+'_{a,s}/R_N').scale(0.68)
