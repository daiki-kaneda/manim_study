...
class LocalUniformPacking(PacedScene):
    """#1840 局所一様パッキング（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("局所一様パッキング")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("局所一様パッキング", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("半径球一様", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq = MathTex('M_{lu}('+chr(92)+"epsilon"+')').scale(0.62)
        ...
        self.read(0.2)
        eq2 = MathTex('M_{lu}'+chr(92)+"le"+' M_u').scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex('M_{lu}'+chr(92)+"le"+' M_u').scale(0.68)
