...
class TernaryTreeRefine(PacedScene):
    """#1841 三分木細分（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("三分木細分")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("三分木", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("三分岐", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq2 = MathTex('T(n,k;3)').scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex('T(n,k;3)').scale(0.68)
