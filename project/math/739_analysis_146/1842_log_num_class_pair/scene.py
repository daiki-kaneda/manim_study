...
class LogNumClassPair(PacedScene):
    """#1842 ログ数値類対（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("ログ数値類対")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("ログ数値類", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("数値同値", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq = MathTex('D'+chr(92)+"equiv"+" D'").scale(0.62)
