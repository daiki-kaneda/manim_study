...
class LogPicardPair(PacedScene):
    """#1844 ログピカール対（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("ログピカール対")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("ログピカール", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("直線束", font_size=24).move_to(self.note)
