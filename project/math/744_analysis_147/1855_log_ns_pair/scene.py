...
class LogNsPair(PacedScene):
    """#1855 ログネロンセベリ対（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("ログネロンセベリ対")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("ログネロンセベリ", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("数値ピカール", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq2 = MathTex(chr(92)+"rho").scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex(chr(92)+"rho"+'(X)').scale(0.68)
