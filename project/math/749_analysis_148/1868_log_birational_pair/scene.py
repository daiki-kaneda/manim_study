...
class LogBirationalPair(PacedScene):
    """#1868 ログ双有理対（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("ログ双有理対")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("ログ双有理", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("双有理写像", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq = MathTex('X'+chr(92)+"dashrightarrow"+' Y').scale(0.62)
        ...
        self.read(0.2)
        eq2 = MathTex('(X,D)'+chr(92)+"dashrightarrow"+"(Y,D')").scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex('(X,D)'+chr(92)+"dashrightarrow"+"(Y,D')").scale(0.68)
