...
class LogProjPair(PacedScene):
    """#1879 ログ射影対（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("ログ射影対")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("ログ射影", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("射影埋め込み", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq2 = MathTex('X'+chr(92)+"hookrightarrow"+chr(92)+"mathbb"+'{P}^N').scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex('X'+chr(92)+"hookrightarrow"+chr(92)+"mathbb"+'{P}^N').scale(0.68)
