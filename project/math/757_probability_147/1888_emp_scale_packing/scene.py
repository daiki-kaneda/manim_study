...
class EmpScalePacking(PacedScene):
    """#1888 経験スケールパッキング（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("経験スケールパッキング")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("経験スケールパッキング", font_size=24)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq2 = MathTex('M_n'+chr(92)+"propto"+'(r/'+chr(92)+"epsilon"+')^d').scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex('M_n'+chr(92)+"propto"+'(r/'+chr(92)+"epsilon"+')^d').scale(0.68)
