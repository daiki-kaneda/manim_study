...
class LogAlbanesePair(PacedScene):
    """#1892 ログアルバネーゼ対（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("ログアルバネーゼ対")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("ログアルバネーゼ", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("アルバネーゼ射", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq2 = MathTex('alb:X'+chr(92)+"to"+' Alb').scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex('alb:X'+chr(92)+"to"+' Alb').scale(0.68)
