...
class UniformScalePacking(PacedScene):
    """#1900 一様スケールパッキング（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("一様スケールパッキング")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("一様スケールパッキング", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("全空間半径", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq = MathTex('M_u(r,'+chr(92)+"epsilon"+')').scale(0.62)
        ...
        self.read(0.2)
        eq2 = MathTex('M_u'+chr(92)+"propto"+'(r/'+chr(92)+"epsilon"+')^d').scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex('M_u'+chr(92)+"propto"+'(r/'+chr(92)+"epsilon"+')^d').scale(0.68)
