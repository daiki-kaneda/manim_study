...
class CayleyForestRefine(PacedScene):
    """#1901 ケイリー森細分（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("ケイリー森細分")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("ケイリー森", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("森の公式", font_size=24).move_to(self.note)
    ...
    def show_formula(self):
        formula = MathTex('CF(n,k)').scale(0.68)
