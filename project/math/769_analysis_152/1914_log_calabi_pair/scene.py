...
class LogCalabiPair(PacedScene):
    """#1914 ログカラビ対（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("ログカラビ対")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("ログカラビ", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("リッチ平坦", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq = MathTex('Ric('+chr(92)+"omega"+')').scale(0.62)
        ...
        self.read(0.2)
        eq2 = MathTex('Ric('+chr(92)+"omega"+')=0').scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex('Ric('+chr(92)+"omega"+')=0').scale(0.68)
