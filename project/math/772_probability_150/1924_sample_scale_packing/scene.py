...
class SampleScalePacking(PacedScene):
    """#1924 標本スケールパッキング（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("標本スケールパッキング")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("標本スケールパッキング", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("有限標本半径", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq = MathTex('M_n(r,'+chr(92)+"epsilon"+')').scale(0.62)
