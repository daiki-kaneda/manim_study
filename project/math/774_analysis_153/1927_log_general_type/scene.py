...
class LogGeneralType(PacedScene):
    """#1927 ログ一般型対（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("ログ一般型対")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("ログ一般型", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("ビッグ標準", font_size=24).move_to(self.note)
