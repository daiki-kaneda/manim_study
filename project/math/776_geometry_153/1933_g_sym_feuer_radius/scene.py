...
class GSymFeuerRadius(PacedScene):
    """#1933 重心類似フォイエル半径比（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("重心類似フォイエル半径比")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("重心類似半径", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("九点円半径", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq2 = MathTex(chr(92)+"ell"+'/R_{N,s}').scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex(chr(92)+"ell"+'/R_{N,s}').scale(0.68)
