...
class NSymFeuerRadius(PacedScene):
    """#1934 九点類似フォイエル半径比（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("九点類似フォイエル半径比")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("九点円類似半径", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("半径", font_size=24).move_to(self.note)
    ...
    def show_formula(self):
        formula = MathTex('R_{N,s}=R/2').scale(0.68)
