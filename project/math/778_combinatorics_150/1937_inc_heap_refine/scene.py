...
class IncHeapRefine(PacedScene):
    """#1937 増加ヒープ細分（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("増加ヒープ細分")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("増加ヒープ", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("ヒープ増加", font_size=24).move_to(self.note)
    ...
    def show_formula(self):
        formula = MathTex('IH(n,k)').scale(0.68)
