...
class LogKodairaPair(PacedScene):
    """#1938 ログ小平対（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("ログ小平対")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("ログ小平", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("次元分類", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq = MathTex(chr(92)+"kappa"+'(X,K+D)').scale(0.62)
        ...
        self.read(0.2)
        eq2 = MathTex(chr(92)+"kappa"+chr(92)+"in"+chr(92)+"{"+'-'+chr(92)+"infty"+',0,'+chr(92)+"ldots"+',n'+chr(92)+"}").scale(0.62)
        eq2.move_to(eq)
