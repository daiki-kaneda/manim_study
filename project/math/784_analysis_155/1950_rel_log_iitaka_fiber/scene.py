...
class RelLogIitakaFiber(PacedScene):
    """#1950 相対ログ飯高ファイバー対（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("相対ログ飯高ファイバー対")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("相対ログ飯高ファイバー", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("相対飯高射", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq2 = MathTex('F='+chr(92)+"phi"+'^{-1}(pt)').scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex('F='+chr(92)+"phi"+'^{-1}(pt)').scale(0.68)
