...
class LogAdjointPair(PacedScene):
    """#1951 ログ随伴対（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("ログ随伴対")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("ログ随伴", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("随伴環", font_size=24).move_to(self.note)
    ...
    def show_formula(self):
        formula = MathTex('R(K+D)').scale(0.68)
