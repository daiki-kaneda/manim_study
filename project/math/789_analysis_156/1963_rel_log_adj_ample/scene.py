...
class RelLogAdjAmple(PacedScene):
    """#1963 相対ログ随伴豊富対（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("相対ログ随伴豊富対")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("相対ログ随伴豊富", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("相対随伴豊富", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq2 = MathTex('(K+D)|_F>0').scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex('(K+D)|_F>0').scale(0.68)
