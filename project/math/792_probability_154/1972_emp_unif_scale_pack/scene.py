...
class EmpUnifScalePack(PacedScene):
    """#1972 経験一様スケールパッキング（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("経験一様スケールパッキング")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("経験一様スケールパッキング", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("データ一様半径", font_size=24).move_to(self.note)
