...
class RelLogAdjNef(PacedScene):
    """#1974 相対ログ随伴ネフ対（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("相対ログ随伴ネフ対")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("相対ログ随伴ネフ", font_size=24)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq2 = MathTex('(K+D).C'+chr(92)+"ge"+' 0').scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex('(K+D).C'+chr(92)+"ge"+' 0').scale(0.68)
