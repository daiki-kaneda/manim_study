...
class OrderedTernaryRefine(PacedScene):
    """#1985 順序三分木細分（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("順序三分木細分")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("順序三分木", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("三分岐順序", font_size=24).move_to(self.note)
    ...
    def show_formula(self):
        formula = MathTex('OT(n,k)').scale(0.68)
