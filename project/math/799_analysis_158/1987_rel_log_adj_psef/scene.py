...
class RelLogAdjPsef(PacedScene):
    """#1987 相対ログ随伴擬有効対（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("相対ログ随伴擬有効対")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("相対ログ随伴擬有効", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("相対閉錐", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq = MathTex(chr(92)+"overline"+'{Eff}_{X/S}').scale(0.62)
        eq.to_edge(DOWN, buff=0.2)
        self.play(Write(eq), run_time=1.35)
        self.read(0.3)
        cap2 = self.ja_text("閉包", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap2), run_time=0.75)
        self.read(0.2)
        eq2 = MathTex('K+D'+chr(92)+"in"+chr(92)+"overline"+'{Eff}').scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex('K+D'+chr(92)+"in"+chr(92)+"overline"+'{Eff}').scale(0.68)
