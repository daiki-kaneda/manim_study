...
class PlaneIncForestRefine(PacedScene):
    """#1997 平面増加森細分（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("平面増加森細分")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def mid(self):
        cap = self.ja_text("順序増加", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq2 = MathTex('PIF(n,k)').scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex('PIF(n,k)').scale(0.68)
