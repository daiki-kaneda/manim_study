...
class RelLogAdjMov(PacedScene):
    """#1998 相対ログ随伴移動対（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("相対ログ随伴移動対")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("相対ログ随伴移動", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("相対移動", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq = MathTex('Mov_{X/S}').scale(0.62)
        ...
        self.read(0.2)
        eq2 = MathTex('K+D'+chr(92)+"in"+' Mov').scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex('K+D'+chr(92)+"in"+' Mov').scale(0.68)
