...
class ScaleUnifCovRev(PacedScene):
    """#2008 スケール一様被覆再訪（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("スケール一様被覆再訪")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("スケール一様被覆", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("再訪", font_size=24).move_to(self.note)
