...
class RelLogAdjVol(PacedScene):
    """#2011 相対ログ随伴体積対（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("相対ログ随伴体積対")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("相対ログ随伴体積", font_size=24)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq2 = MathTex('vol_{X/S}(K+D)').scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex('vol_{X/S}(K+D)').scale(0.68)
