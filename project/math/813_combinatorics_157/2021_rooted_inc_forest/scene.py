...
class RootedIncForest(PacedScene):
    """#2021 根つき増加森細分（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("根つき増加森細分")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("根つき増加森", font_size=24)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq2 = MathTex('RIF(n,k)').scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex('RIF(n,k)').scale(0.68)
