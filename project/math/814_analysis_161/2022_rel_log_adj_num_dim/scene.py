...
class RelLogAdjNumDim(PacedScene):
    """#2022 相対ログ随伴数値次元対（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("相対ログ随伴数値次元対")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("相対ログ随伴数値次元", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("相対成長", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq = MathTex(chr(92)+"kappa"+'_'+chr(92)+"sigma"+'(X/S,K+D)').scale(0.62)
        ...
        self.read(0.2)
        eq2 = MathTex(chr(92)+"kappa"+'_'+chr(92)+"sigma").scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex(chr(92)+"kappa"+'_'+chr(92)+"sigma"+'(X/S,K+D)').scale(0.68)
