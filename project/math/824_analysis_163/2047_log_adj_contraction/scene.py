...
class LogAdjContraction(PacedScene):
    """#2047 ログ随伴収縮対（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("ログ随伴収縮対")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("ログ随伴収縮", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("例外因子", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq = MathTex(chr(92)+"mathrm"+'{Exc}('+chr(92)+"phi"+')').scale(0.62)
