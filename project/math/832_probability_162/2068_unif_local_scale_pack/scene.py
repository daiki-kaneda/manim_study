...
class UnifLocalScalePack(PacedScene):
    """#2068 一様局所スケールパッキング（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("一様局所スケールパッキング")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("一様局所スケールパッキング", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("一様局所", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq = MathTex('M_{n,u}(B,r,'+chr(92)+"epsilon"+')').scale(0.62)
