...
class RootedLabeledIncTree(PacedScene):
    """#2069 根つきラベル増加木細分（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("根つきラベル増加木細分")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        vals = VGroup(*[
            ...
            for i, v in enumerate([1, 1, 4, 20, 124])
        ])
        self.play(LaggedStart(*[FadeIn(v) for v in vals], lag_ratio=0.1), run_time=1.4)
        note = self.ja_text("根つきラベル増加木", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("根とラベル", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq2 = MathTex('RLI(n,k)').scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex('RLI(n,k)').scale(0.68)
