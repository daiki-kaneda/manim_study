...
class LogAdjFinal(PacedScene):
    """#2071 ログ随伴終局対（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("ログ随伴終局対")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("ログ随伴終局", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("終局モデル", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq2 = MathTex('(X,D)'+chr(92)+" "+chr(92)+"mathrm"+'{final}').scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex('(X,D)'+chr(92)+" "+chr(92)+"mathrm"+'{final}').scale(0.68)
