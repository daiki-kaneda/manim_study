...
class EmpDataRadiusPack(PacedScene):
    """#2080 経験データ半径パッキング（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("経験データ半径パッキング")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("経験データ半径パッキング", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("経験データ", font_size=24).move_to(self.note)
        self.play(Transform(self.note, cap), run_time=0.9)
        self.read(0.25)
        cap2 = self.ja_text("分離", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq = MathTex('M_n^{'+chr(92)+"mathrm"+'{data}}(r,'+chr(92)+"epsilon"+')').scale(0.62)
        ...
        self.read(0.2)
        eq2 = MathTex('M_n^{'+chr(92)+"mathrm"+'{data}}'+chr(92)+"lesssim"+'(r/'+chr(92)+"epsilon"+')^d').scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex('M_n^{'+chr(92)+"mathrm"+'{data}}'+chr(92)+"lesssim"+'(r/'+chr(92)+"epsilon"+')^d').scale(0.68)
