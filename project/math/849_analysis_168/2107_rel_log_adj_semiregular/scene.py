...
class RelLogAdjSemiregular(PacedScene):
    """#2107 相対ログ随伴準正則対（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("相対ログ随伴準正則対")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("相対ログ随伴準正則", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("相対準正則", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq2 = MathTex('(X/S,D)'+chr(92)+" "+chr(92)+"mathrm"+'{semiregular}').scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex('(X/S,D)'+chr(92)+" "+chr(92)+"mathrm"+'{semiregular}').scale(0.68)
