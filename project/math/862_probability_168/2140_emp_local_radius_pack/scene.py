...
class EmpLocalRadiusPack(PacedScene):
    """#2140 経験局所半径パッキング（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("経験局所半径パッキング")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("経験局所半径パッキング", font_size=24)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq = MathTex('M_n^{el}(r,'+chr(92)+"epsilon"+')').scale(0.62)
        ...
        self.read(0.2)
        eq2 = MathTex('M_n^{el}'+chr(92)+"lesssim"+'(r/'+chr(92)+"epsilon"+')^d').scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex('M_n^{el}'+chr(92)+"lesssim"+'(r/'+chr(92)+"epsilon"+')^d').scale(0.68)
