...
class PlanarIncGrove(PacedScene):
    """#2237 平面増加林細分（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("平面増加林細分")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("平面増加林", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("平面林", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq2 = MathTex('PIG(n,k)').scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex('PIG(n,k)').scale(0.68)
