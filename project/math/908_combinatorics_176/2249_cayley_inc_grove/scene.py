...
class CayleyIncGrove(PacedScene):
    """#2249 ケイリー増加林細分（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("ケイリー増加林細分")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        vals = VGroup(*[
            ...
            for i, v in enumerate([1, 3, 16, 125, 1296])
        ])
        self.play(LaggedStart(*[FadeIn(v) for v in vals], lag_ratio=0.1), run_time=1.4)
        note = self.ja_text("ケイリー増加林", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("ケイリー林", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq2 = MathTex('CIG(n,k)').scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex('CIG(n,k)').scale(0.68)
