...
class LogAdjSemiflip(PacedScene):
    """#2250 ログ随伴準フリップ対（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("ログ随伴準フリップ対")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("ログ随伴準フリップ", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("準フリップ", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq = MathTex(chr(92)+"phi"+'^-_{'+chr(92)+"mathrm"+'{semi}}').scale(0.62)
        ...
        self.read(0.2)
        eq2 = MathTex('X'+chr(92)+"dashrightarrow"+' X^+').scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex('X'+chr(92)+"dashrightarrow"+' X^+').scale(0.68)
