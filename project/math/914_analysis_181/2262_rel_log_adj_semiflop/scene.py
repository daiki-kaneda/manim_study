...
class RelLogAdjSemiflop(PacedScene):
    """#2262 相対ログ随伴準フロップ対（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("相対ログ随伴準フロップ対")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("相対ログ随伴準フロップ", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("相対準フロップ", font_size=24).move_to(self.note)
