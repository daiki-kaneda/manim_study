...
class LocalRadiusDataCov(PacedScene):
    """#2271 局所半径データ被覆（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("局所半径データ被覆")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("局所半径データ被覆", font_size=24)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq = MathTex('N_n^{lrd}(r,'+chr(92)+"epsilon"+')').scale(0.62)
        ...
        self.read(0.2)
        eq2 = MathTex('N_n^{lrd}'+chr(92)+"lesssim"+'(r/'+chr(92)+"epsilon"+')^d').scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex('N_n^{lrd}'+chr(92)+"lesssim"+'(r/'+chr(92)+"epsilon"+')^d').scale(0.68)
