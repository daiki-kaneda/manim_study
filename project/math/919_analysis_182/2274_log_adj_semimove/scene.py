...
class LogAdjSemimove(PacedScene):
    """#2274 ログ随伴準移動対（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("ログ随伴準移動対")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("ログ随伴準移動", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("準移動", font_size=24).move_to(self.note)
