...
class TernaryIncRootGrove(PacedScene):
    """#2285 三分増加根林細分（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("三分増加根林細分")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def mid(self):
        cap = self.ja_text("三分根林", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq2 = MathTex('TIRG(n,k)').scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex('TIRG(n,k)').scale(0.68)
