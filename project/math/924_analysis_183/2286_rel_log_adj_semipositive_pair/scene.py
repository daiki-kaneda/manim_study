...
class RelLogAdjSemipositivePair(PacedScene):
    """#2286 相対ログ随伴準正値対（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("相対ログ随伴準正値対")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("相対ログ随伴準正値", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("相対準正値", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq = MathTex('(K+D)'+chr(92)+"cdot"+' C>0').scale(0.62)
