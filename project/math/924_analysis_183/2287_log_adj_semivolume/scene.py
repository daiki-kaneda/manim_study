...
class LogAdjSemivolume(PacedScene):
    """#2287 ログ随伴準体積対（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("ログ随伴準体積対")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("ログ随伴準体積", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("準体積", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq = MathTex('vol_{'+chr(92)+"mathrm"+'{semi}}(K+D)').scale(0.62)
        ...
        self.read(0.2)
        eq2 = MathTex('vol_{'+chr(92)+"mathrm"+'{semi}}(K+D)').scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex('vol_{'+chr(92)+"mathrm"+'{semi}}(K+D)').scale(0.68)
