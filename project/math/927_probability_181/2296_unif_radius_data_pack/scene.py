...
class UnifRadiusDataPack(PacedScene):
    """#2296 一様半径データパッキング（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("一様半径データパッキング")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("一様半径データパッキング", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("一様半径", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq = MathTex('M_n^{urd}(r,'+chr(92)+"epsilon"+')').scale(0.62)
        ...
        self.read(0.2)
        eq2 = MathTex('M_n^{urd}'+chr(92)+"lesssim"+'(r/'+chr(92)+"epsilon"+')^d').scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex('M_n^{urd}'+chr(92)+"lesssim"+'(r/'+chr(92)+"epsilon"+')^d').scale(0.68)
