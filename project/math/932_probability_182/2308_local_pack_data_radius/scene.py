...
class LocalPackDataRadius(PacedScene):
    """#2308 局所パッキングデータ半径（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("局所パッキングデータ半径")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("局所パッキングデータ半径", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("パッキング半径", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq = MathTex('r_n^{'+chr(92)+"mathrm"+'{pack}}').scale(0.62)
    ...
    def show_formula(self):
        formula = MathTex('M'+chr(92)+"lesssim"+'(r_n^{'+chr(92)+"mathrm"+'{pack}}/'+chr(92)+"epsilon"+')^d').scale(0.68)
