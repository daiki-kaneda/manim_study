...
class UnifCovDataRadius(PacedScene):
    """#2331 一様被覆データ半径（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("一様被覆データ半径")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("一様被覆データ半径", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("一様被覆", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq = MathTex('r_n^{'+chr(92)+"mathrm"+'{ucov}}').scale(0.62)
    ...
    def show_formula(self):
        formula = MathTex('N'+chr(92)+"lesssim"+'(r_n^{'+chr(92)+"mathrm"+'{ucov}}/'+chr(92)+"epsilon"+')^d').scale(0.68)
