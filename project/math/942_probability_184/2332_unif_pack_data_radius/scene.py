...
class UnifPackDataRadius(PacedScene):
    """#2332 一様パッキングデータ半径（約45秒・導出つき）"""

    def construct(self):
        self.show_heading("一様パッキングデータ半径")
        self.draw()
        self.mid()
        self.derive()
        self.show_formula()
    ...
    def draw(self):
        ...
        note = self.ja_text("一様パッキングデータ半径", font_size=24)
    ...
    def mid(self):
        cap = self.ja_text("一様パッキング", font_size=24).move_to(self.note)
    ...
    def derive(self):
        ...
        self.read(0.2)
        eq = MathTex('r_n^{'+chr(92)+"mathrm"+'{upack}}').scale(0.62)
        ...
        self.read(0.2)
        eq2 = MathTex('M'+chr(92)+"lesssim"+'(r/'+chr(92)+"epsilon"+')^d').scale(0.62)
        eq2.move_to(eq)
    ...
    def show_formula(self):
        formula = MathTex('M'+chr(92)+"lesssim"+'(r_n^{'+chr(92)+"mathrm"+'{upack}}/'+chr(92)+"epsilon"+')^d').scale(0.68)
